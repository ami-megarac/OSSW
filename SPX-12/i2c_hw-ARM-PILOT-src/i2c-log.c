...
static int log_i2c_proc_read(struct file *file,char __user *buffer, size_t buffer_length,loff_t *offset)
#else
static int log_i2c_proc_read(char *buffer, char **buffer_location,off_t offset, int buffer_length, int *eof, void *data)
#endif
{
	struct log_i2c_conf *log_conf = NULL;
	int len;
	u8 i;
	...
	log_conf=PDE_DATA(file_inode(file));
#else
	*eof = 1;
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(3,4,11))
	static int log_i2c_proc_write(struct file *file,const char *buffer,size_t buffer_length,loff_t *offp)
#else
	static int log_i2c_proc_write(struct file *file, const char __user *buffer,unsigned long buffer_length, void *data)
#endif
{
	struct log_i2c_conf *log_conf = NULL;
	char *str, *p, *line;
...
{
	struct i2c_pilot_ii_data *algo_data;
	...
	proc_fops->read = log_i2c_proc_read;
	proc_fops->write = log_i2c_proc_write;
	
	pde = proc_create_data(name, 0644, NULL,proc_fops,log_conf);
	if (!pde)
	{
