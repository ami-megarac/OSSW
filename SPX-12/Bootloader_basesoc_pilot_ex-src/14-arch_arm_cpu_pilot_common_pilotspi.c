diff -Naur uboot/arch/arm/cpu/pilotcommon/pilotspi.c uboot_new/arch/arm/cpu/pilotcommon/pilotspi.c
--- uboot/arch/arm/cpu/pilotcommon/pilotspi.c	1969-12-31 19:00:00.000000000 -0500
+++ uboot_new/arch/arm/cpu/pilotcommon/pilotspi.c	2016-09-30 13:55:12.121842333 -0400
@@ -0,0 +1,816 @@
+/*
...
+	if ((cmd[0]==0x6b)||(cmd[0] == 0x0B)) //Quad & fast read
+	{
+		if (cmdlen == 6) addrmode = 4; // change to 32-bits address mode
+	}
+	else if (cmd[0] == 0xEB) // 4xIO read
+	{
+		addrmode = 4; // change to 32-bits address mode
+	}
+	else
+	{
...
+
+#endif
+	if((Opcode==0x6b)||(Opcode==0x32))//||(Opcode==0xd8))//||(Opcode==0x02))
+	{
...
+  if (flash_info[bank].size >= FLASH_128MB) flsize = CS_128MB;
+  
+  if (bank == 0)
+  {
+    Regs->bmisc &= ~(CS0_MASK);
...
+    #else
+    .operation_mode_mask = 0x00010001, //Default
+    #endif
+	.fast_read	= 1,
...
+    pilot3spi_bkup_init();
+    p_soc_spi_transfer = (p_soc_spi_transfer_t) &pilot3spi_transfer;
+	return;
+}
+
+#else		/* Linux */
