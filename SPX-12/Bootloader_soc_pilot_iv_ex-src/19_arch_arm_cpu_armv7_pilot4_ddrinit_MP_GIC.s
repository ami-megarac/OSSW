--- u-boot-2013.07/arch/arm/cpu/armv7/pilot4/ddrinit/MP_GIC.s	1969-12-31 19:00:00.000000000 -0500
+++ mywork/arch/arm/cpu/armv7/pilot4/ddrinit/MP_GIC.s	2015-02-06 05:06:37.000000000 -0500
@@ -0,0 +1,384 @@
+@; ------------------------------------------------------------
...
+  MRC     p15, 4, r5, c15, c0, 0  @; Read periph base address samar reading Configuration Base address reg pg no 91 a9 trm
+  ADD     r5, r5, #0x1000         @; Add the GIC offset samar ref table 1-3 a9 mpcore trm Interrupt distributor interrupt sources
+  MOV     r0, r0, LSR #2
+  ORR	  r7, r0, #0x3
+  CMP     r7, #0x0
+  BEQ     do_aligned
...
+  MOV	  r1, r1, LSL r6
+  LDR	  r6, [r5]
+  ORR	  r1, r1, r6 @; Read modify write the priority level value
+  STR	  r1, [r5]
+  BX      lr
+
+doaddprio:
...
+  MOV	  r3, #0x0
+  MOV	  r4, r4, LSL r1 
+  MOV     r3, r4 @; CPU Targets Byte offset 0
+  ORR     r3, r4, LSL #8 @; CPU Targets Byte offset 1
+  ORR     r3, r4, LSL #16 @; CPU Targets Byte offset 2
+  ORR     r3, r4, LSL #24 @; CPU Targets Byte offset 2
+  STR     r3, [r5]
+  @;SUB     r5, r5, #0x400 
+  @;LDR     r4, [r5] @; Just read the priority register at this point
+  @;LDR	  r3, =0xFFFFFFFF
+  @;STR     r3, [r5]
+  @;LDR	  r3, [r5]
+  @;LDR	  r3, =0x08080808
...
+
+  @; Get base address of private perpherial space
+  MRC     p15, 4, r0, c15, c0, 0  @; Read periph base address samar reading Configuration Base address reg pg no 91 a9 trm
+  ADD     r0, r0, #0x1000         @; Add the GIC offset samar ref table 1-3 a9 mpcore trm Interrupt distributor interrupt sources                                                          
+
+  LDR     r1, [r0]                @; Read the GIC's Enable Register  (ICDDCR) samar ref pg 64 gic arch spec                                                              
...
+  STR     r3, [r0, r2]            @; Store out  (ICDISER) samar ref to table 1-3 a9 mpcore trm, 4.3.5 pg 71 gic arch spec                                                                
+
+  BX      lr
+
+@; ------------------------------------------------------------
...
+                                  @; samar ref to table 4-3 gic arch spec table 1-3, 3-1 a9 mp core trm                                                                                        
...
+  STR     r3, [r0]                @; And store it back again  (ICDIPR) samar ref to pg 83 4.3.10, table 4-3 gic arch spec table 1-3, 3-1 a9 mp core trm
+
+  BX      lr
+
+@; ------------------------------------------------------------
...
+  MRC     p15, 4, r0, c15, c0, 0  @; Read periph base address  samar reading Configuration Base address reg pg no 91 a9 trm  
+
+  LDR     r1, [r0, #0x100]        @; Read the Processor Interface Control register   (ICCICR/ICPICR) samar ref to table 1-3, 3-8 a9 mpcore trm, 4.4.1 pg 99 GIC arch spec 
+  ORR     r1, r1, #0x03           @; Bit 0: Enables secure interrupts, Bit 1: Enables Non-Secure interrupts
+  STR     r1, [r0, #0x100]        @; Write the Processor Interface Control register  (ICCICR/ICPICR) samar ref to GIC arch spec
+
+  BX      lr
+
+
...
+  MRC     p15, 4, r0, c15, c0, 0  @; Read periph base address
+  
+  STR     r1, [r0, #0x0104]       @; Write the Priority Mask register (ICCPMR/ICCIPMR) samar ref to table 1-3, 3-8 a9 mpcore trm , 4.4.2 pg 104 gic arch spec                                                                 
+  BX      lr
+  
+@; ------------------------------------------------------------
+
