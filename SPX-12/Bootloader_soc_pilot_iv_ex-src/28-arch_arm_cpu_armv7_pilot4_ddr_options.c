--- u-boot-2013.07/arch/arm/cpu/armv7/pilot4/ddr_options.c	1969-12-31 19:00:00.000000000 -0500
+++ mywork/arch/arm/cpu/armv7/pilot4/ddr_options.c	2015-09-25 07:13:55.176438550 -0400
@@ -0,0 +1,5139 @@
+/*******************************************************************************
+ *
...
+//|                    |                            |  be defined |                                |
+//|------------------------------------------------------------------------------------------------|
+//|     1600 Mbps      |     DDR_1600MHZ           |  NO         | DDR3 - WL8RL11 DDR4 - WL11RL12 |
+//|------------------------------------------------------------------------------------------------|
+//|     1300 Mbps      |     DDR_1333MHZ           |  NO         | DDR3 - WL7RL9 DDR4 - WL9RL10   |
+//|------------------------------------------------------------------------------------------------|
+//|     1000 Mbps      |     DDR_1066MHZ           |  DDR3 - NO  | DDR3 - WL6RL7 DDR4 - WL9RL10   |
+//|                    |                            |  DDR4 - YES |                                |
+//|------------------------------------------------------------------------------------------------|
+//|     800 Mbps       |     DDR_800MHZ            |  DDR3 - NO  | DDR3 - WL5RL6 DDR4 - WL9RL10   |
+//|                    |                            |  DDR4 - YES |                                |
+//|------------------------------------------------------------------------------------------------|
+//|     500 Mbps       |     DDR_500MHZ            |  YES        | DDR3 - WL6RL6 DDR4 - WL9RL10   |
+//|------------------------------------------------------------------------------------------------|
+//|     400 Mbps       |     DDR_350_400MHZ        |  YES        | DDR3 - WL6RL6 DDR4 - WL9RL10   |
+//|------------------------------------------------------------------------------------------------|
+//|     200 Mbps       |     DDR_200_250MHZ        |  YES        | DDR3 - WL6RL6 DDR4 - WL9RL10   |
+//|------------------------------------------------------------------------------------------------|
+//|     100 Mbps       |     DDR_75_125MHZ         |  YES        | DDR3 - WL6RL6 DDR4 - WL9RL10   |
+// -------------------------------------------------------------------------------------------------
+// SKIP_GATE_TRAINING - Enable only for Simulation.
+// SKIP_EYE_TRAINING - Enable only for Simulation. For DDR_200_250MHZ, DDR_75_125MHZ.
...
+// Pass DDRPHY_ASSEERT_UPDREQ define(as command line agrument) along with enabling ZQCALIB_SHORT_INTERVAL define (in this file) to simulate ZQCalib at regular intervals. 
+//((((((((((((((((((((((((((((((((((((((((((((((((((((((((((((((((((((((((((((((((((((((((((((((((((
+// *************************************************************************************************
+#include "include/pilot_II.h"
+
...
+//#define DDR3                1
+//#define DDR3L               1
+//#define DDR_75_125MHZ      1
+//#define DDR_200_250MHZ     1
+//#define DDR_350_400MHZ     1
+//#define DDR_500MHZ         1
+//#define DDR_800MHZ         1
+//#define DDR_1066MHZ        1
+//#define DDR_1333MHZ        1
+//#define DDR_1600MHZ        1
+//#define WL5RL6              1
+//#define WL6RL6              1
+//#define WL6RL7              1
+//#define WL7RL9              1
...
+//#define HW_ENVPRN            1
+
+#define LMEMDDRTRAINVAR 0x10004F00 //0x40100200
+//volatile char * LMEMDDRTRAINVAR = 0x40100200; // Global array (1024 Bytes used).
+//unsigned int LMEMDDRTRAINVAR[1000]={0};
+
+//#define MORE_PRINT			1
+#define DEFAULT_PRINTS                  1 //This define is used in generating DDR initialization results when u-boot is executed.
+//#define ZQCALIB_SHORT_INTERVAL                1
+#define P4_WR_ARB 1  //When enabled, changes the WR arbitration weight from 4 to 1
+
+#define SERIAL_LSR_THRE		0x20    /* THR Empty */
+#define UART_BASE			0x40430000
...
+}
+
+static void print_dec(unsigned int reg)
+{
+	unsigned int i=0;
...
+}
+
+tVU16 VrefPN_array1(tU16 i, tU16 j)
+{
+	tU16 val;
+	if(j==0)
+		val = i*2;
...
+#endif
+
+void get_arrays(tU8 ddr_type, tPVU32 arr)
+{
+	if(!ddr_type)
+	{
+		// First 8 are DDR3, next 8 are DDR4. Frequencies Highest to lowest.
+#if defined (DDR_1600MHZ) 
+		*(tPVU32)(arr++) = 0x12;
+		*(tPVU32)(arr++) = 0xa;
+		*(tPVU32)(arr) = 0x0194;
+#endif
+#if defined (DDR_1333MHZ)
+		*(tPVU32)(arr++) = 0x0e;
+		*(tPVU32)(arr++) = 0x08;
+		*(tPVU32)(arr) = 0x0151;
+#endif
+#if defined (DDR_1066MHZ)
+		*(tPVU32)(arr++) = 0x0b;
+		*(tPVU32)(arr++) = 0x06;
+		*(tPVU32)(arr) = 0xed;
+#endif
+#if defined (DDR_800MHZ )
+		*(tPVU32)(arr++) = 0x08;
+		*(tPVU32)(arr++) = 0x05;
+		*(tPVU32)(arr) = 0xaa;
+#endif
+#if defined (DDR_500MHZ)
+		*(tPVU32)(arr++) = 0x05;
+		*(tPVU32)(arr++) = 0x03;
+		*(tPVU32)(arr) = 0x67;
+#endif
+#if defined (DDR_350_400MHZ)
+		*(tPVU32)(arr++) = 0x03;
+		*(tPVU32)(arr++) = 0x03;
+		*(tPVU32)(arr) = 0x45;
+#endif
+#if defined (DDR_200_250MHZ)
+		*(tPVU32)(arr++) = 0x01;
+		*(tPVU32)(arr++) = 0x02;
+		*(tPVU32)(arr) = 0x23;
+#endif
+#if defined (DDR_75_125MHZ)
+		*(tPVU32)(arr++) = 0x01;
+		*(tPVU32)(arr++) = 0x01;
+		*(tPVU32)(arr) = 0x22;
+#endif
...
+#endif
+
+tVU8 array(int ddr_type, int idx, int x, int y)
+{
+	tVU8 val;
+	if(!ddr_type)
+	{
...
+  
+#if defined (CONFIG_SPX_FEATURE_GLOBAL_MEMORY_ECC_ENABLE)
+  // Lower ECC address
...
+  tU32 i, sedid;
+  tVU8 ddr_type, p4_rev;
+
+  tU16 dqsgate_array[512];
+  tU16 VrefPN_array[256][2];
+
+  tVU8 WL8RL11, WL11RL12, WL9RL10, WL7RL9, WL6RL7, DLL_OFF, WL5RL6, WL6RL6;
+#ifndef ASIC_BUILD
+	tU32 brcl_version = BRCL_VERSION; //This is version number of ddr_options.c maintained by Broadcom 
+	tU32 version = DDROPVERSION; //Customer version number of ddr_options.c. This is used by customer and entered as a command line  argument when building binary
+
+	putc('\n'); putc('\r');  putc('B'); putc('R'); putc('C'); putc('L'); putc(' '); putc('d'); putc('d');
+	putc('r'); putc('o'); putc('p'); putc('t'); putc('i'); putc('o'); putc('n'); putc('s'); putc('.'); putc('c'); putc(' ');
+	putc('V'); putc('e'); putc('r'); putc('s'); putc('i'); putc('o'); putc('n'); putc(' '); print_reg(brcl_version);
+
+	putc('\n'); putc('\r'); putc('C'); putc('u'); putc('s'); putc('t'); putc('o'); putc('m'); putc('e');
+	putc('r'); putc(' '); putc('V'); putc('e'); putc('r'); putc('s'); putc('i'); putc('o'); putc('n'); putc(' '); 
+	putc('I'); putc('D'); putc(' '); print_reg(version);
+#else
+#if defined (DEFAULT_PRINTS_BOOTLOADER)
+	nc_printf("BRCL ddr_options.c version %x \n", BRCL_VERSION);
+#endif
+#endif
+
+  WL8RL11=0; WL11RL12=0; WL9RL10=0; WL7RL9=0; WL6RL7=0; DLL_OFF=0; WL5RL6=0; WL6RL6=0;
+  ddr_type=1; //by default use DDR4
+
+  i = *(tPVU32)(0x4010010C);
+  if(i & 0x2)
+	ddr_type = 1;
...
+		DLL_OFF = 1;
+#endif
+#if defined (DDR_500MHZ) || defined (DDR_350_400MHZ) || defined (DDR_200_250MHZ) || defined (DDR_75_125MHZ) 
+	if(!ddr_type)
+		WL6RL6 = 1;
+	DLL_OFF = 1;
+#endif
...
+  for(i=0;i<256; i++)
+  {
+ 	VrefPN_array[i][0] = VrefPN_array1(i, 0); 
+ 	VrefPN_array[i][1] = VrefPN_array1(i, 1); 
+  }
+
+  for(i=0;i<512; i++)
+  {
+ 	dqsgate_array[i] = dqsgate_array1(i); 
+  }
+
+  for(i=0;i<108; i++)
+	  *(tPVU8)(LMEMDDRTRAINVAR + i)=0;
+
+
+  //PHY RESET
+  *(tPVU32)(MC_REG_ADDRESS10) = (*(tPVU32)(MC_REG_ADDRESS10) & 0xfffffdff) | 0x00000200;
...
+//#endif
+
+if (WL9RL10==1) { //#if defined (WL9RL10)
+	if(DLL_OFF==1) {//#if defined (DLL_OFF)
+  *(tPVU32)(PHY_REG_ADDRESS00) = (*(tPVU32)(PHY_REG_ADDRESS00) & 0xfffffcf7) | 0x00000308;
+	} else { //#else
+  *(tPVU32)(PHY_REG_ADDRESS00) = (*(tPVU32)(PHY_REG_ADDRESS00) & 0xfffffcf7) | 0x00000208;
+	}//#endif
...
+  *(tPVU32)(APB_REG_ADDRESS00) = (*(tPVU32)(APB_REG_ADDRESS00) & 0x00000000) | 0x803b0301; 
+//#endif
+	} else {
+#if defined (DDR3L)
...
+#endif
+#if defined (DDR_1600MHZ)
+   #if defined (ZQCALIB_SHORT_INTERVAL)
+      *(tPVU32)(APB_REG_ADDRESS08) = (*(tPVU32)(APB_REG_ADDRESS08) & 0x00000000) | 0x00000016;
+   #else
...
+  }
+  else {
+     *(tPVU32)(APB_REG_ADDRESS10) = (*(tPVU32)(APB_REG_ADDRESS10) & 0x00000000) | 0xe7a7f766;//option2
+  }
+#endif
+
+if (WL5RL6==1) //#if defined (WL5RL6)
...
+  *(tPVU32)(APB_REG_ADDRESS14) = (*(tPVU32)(APB_REG_ADDRESS14) & 0x00000000) | 0x000000b1;
+//#endif
+if (WL11RL12==1) //#if defined (WL11RL12)
+  *(tPVU32)(APB_REG_ADDRESS14) = (*(tPVU32)(APB_REG_ADDRESS14) & 0x00000000) | 0x000000b1;
+//#endif
+
...
+	  //*(tPVU32)(APB_REG_ADDRESS20) = (*(tPVU32)(APB_REG_ADDRESS20) & 0x00000000) | 0x02665555;
+	  //#else
+  } else {
...
+	  *(tPVU32)(APB_REG_ADDRESS30) = (*(tPVU32)(APB_REG_ADDRESS30) & 0x00000000) | 0x80180703; // EW //option2
+  } else {
+	  //#else
+	  *(tPVU32)(APB_REG_ADDRESS30) = (*(tPVU32)(APB_REG_ADDRESS30) & 0x00000000) | 0x80180702; // GL
+	  //#endif
...
+  *(tPVU32)(MC_REG_ADDRESS0C) = (*(tPVU32)(MC_REG_ADDRESS0C) & 0xf001ffc0) | 0x0e260022;
+  *(tPVU32)(MC_REG_ADDRESS14) = (*(tPVU32)(MC_REG_ADDRESS14) & 0x01f00000) | 0x50013030;
+#if defined (P4_WR_ARB)
+  *(tPVU32)(MC_REG_ADDRESS28) = (*(tPVU32)(MC_REG_ADDRESS28) & 0x03ff0fff) | 0x24000000;//max_no_of_ahb_stwrs set to 1 and max_no_of_dma_stwr set to 1
+#else
+  *(tPVU32)(MC_REG_ADDRESS28) = (*(tPVU32)(MC_REG_ADDRESS28) & 0x03ff0fff) | 0x90000000;
+#endif
+
+#if defined (P4_WR_ARB)
+  *(tPVU32)(MC_REG_ADDRESS34) = (*(tPVU32)(MC_REG_ADDRESS34) & 0x80000000) | 0x40C92248;//max_no_of_str_stwrs set to 1
+#else 
...
+#if defined (P4_WR_ARB)
+  *(tPVU32)(MC_REG_ADDRESS68) = (*(tPVU32)(MC_REG_ADDRESS68) & 0x40000000) | 0x1f51790A;//max_no_of_axi_stwrs set to 1
+#else 
...
+
+  //
+  *(tPVU32)(MC_REG_ADDRESS1C) = (*(tPVU32)(MC_REG_ADDRESS1C) & 0x40003fff) | 0xbc7f0000;
+
+sedid = *(tPVU32)(0x40100150);
+p4_rev = sedid & 0xFF;
+if(p4_rev == 0x40) {   //ZQ Calibration requst is disabled for Pilot4 A0, A1
+#if defined (HW_ENVPRN)
+    nc_printf("!!! PILOT4 Revision ID is %x \n", p4_rev);
+#endif
+
+if (WL5RL6==1) //#if defined (WL5RL6)
+  *(tPVU32)(PHY_REG_ADDRESS00) = (*(tPVU32)(PHY_REG_ADDRESS00) & 0xffffe778) | 0x00001801;
+//#endif
+if (WL6RL6==1) //#if defined (WL6RL6)
+  *(tPVU32)(PHY_REG_ADDRESS00) = (*(tPVU32)(PHY_REG_ADDRESS00) & 0xffffe778) | 0x00001801;
+//#endif
+if (WL6RL7==1) //#if defined (WL6RL7)
+  *(tPVU32)(PHY_REG_ADDRESS00) = (*(tPVU32)(PHY_REG_ADDRESS00) & 0xffffe778) | 0x00001800;
+//#endif
+if (WL7RL9==1) //#if defined (WL7RL9)
+  *(tPVU32)(PHY_REG_ADDRESS00) = (*(tPVU32)(PHY_REG_ADDRESS00) & 0xffffe778) | 0x00001801;
+//#endif
+//#if defined (WL8RL11)
+if (WL8RL11==1) 
+  *(tPVU32)(PHY_REG_ADDRESS00) = (*(tPVU32)(PHY_REG_ADDRESS00) & 0xffffe778) | 0x00001802;
+//#endif
+if (WL9RL10==1)//#if defined (WL9RL10)
+  *(tPVU32)(PHY_REG_ADDRESS00) = (*(tPVU32)(PHY_REG_ADDRESS00) & 0xffffe778) | 0x00001801; // DLL_OFF change??.
+//#endif
+if (WL11RL12==1) //#if defined (WL11RL12)
+  *(tPVU32)(PHY_REG_ADDRESS00) = (*(tPVU32)(PHY_REG_ADDRESS00) & 0xffffe778) | 0x00001802;
+}
+else { //ZQ Calibration requst is enabled for Pilot4 A2
+#if defined (HW_ENVPRN)
+    nc_printf("!!! PILOT4 Revision ID is %x \n", p4_rev);
+#endif
+if (WL5RL6==1) //#if defined (WL5RL6)
...
+  //{
+//#if defined (DDR4)
+//  *(tPVU32)(APB_REG_ADDRESS34) = (*(tPVU32)(APB_REG_ADDRESS34) & 0x00000000) | 0x04150415; // 1600 and board delay = 1. Actual is 441 with training. //option2
+//#else
+  //} //option2
+  if (!ddr_type) { //option2
+  // DDR3, 1600.
...
+  // DDR3, 800.
+  *(tPVU32)(APB_REG_ADDRESS34) = (*(tPVU32)(APB_REG_ADDRESS34) & 0x00000000) | 0x03190319; // 1600 and board delay = 1. Actual is 319 with training.
+//#endif
+  }
+
+#else
...
+#else
+  if(!ddr_type) { //option2
+  if (gate_training()) { //Gate training for DDR3
+#if defined (HW_ENV)
+	*(tPVU32)0x40430000 = 'E';
+	*(tPVU32)0x40430000 = '1';
+#if defined (HW_ENVPRN)
...
+#endif
+#if defined (DDR_1600MHZ)
+      *(tPVU32)(APB_REG_ADDRESS20C) = 0x80AB8154;
+      *(tPVU32)(APB_REG_ADDRESS210) = 0x80AB8154;
+      *(tPVU32)(APB_REG_ADDRESS144) = 0x0000D5D5;
+      *(tPVU32)(APB_REG_ADDRESS18) = (*(tPVU32)(APB_REG_ADDRESS18) & 0x00000000) | 0x00010101;
+      *(tPVU32)(APB_REG_ADDRESS28) = 0x80AB8154;
+#endif
+
+#else
+
...
+	  if (ddr_type)
+	  {
+		  //Vrefloopmax = 50; //option2
+		  Vrefloopmax = 60; //option2
...
+		  //Vrefstarta = 140; //option2
+		  Vrefstarta = 170; //option2
+	  } else {
+		  //#else
+		  Vrefloopmax = 1;
+		  //#endif
...
+#if defined MORE_PRINT
+	putc('V'); putc('r'); putc('e'); putc('f'); putc('l'); putc('o'); putc('o'); putc('p'); putc('i');putc('n');putc('d');putc('e');putc('x'); putc(' ');print_reg(Vrefloopindex); putc('\n');
+	putc('V'); putc('r'); putc('e'); putc('f'); putc('s'); putc('t'); putc('a'); putc('r'); putc('t');putc('a');putc(' ');print_reg(Vrefstarta); putc('\n');
+#endif	
+//#if defined (DDR4)
+	  if (ddr_type)
+		{
...
+        if (Vrefloopindex == 0) {
+          Vrefdqsrange_best = (*(tPVU32)(LMEMDDRTRAINVAR + 0x58) & 0xff);
+          Vrefdqs_best = Vrefstarta;
+          Vref144Best = *(tPVU32)(APB_REG_ADDRESS144);
+        }
+        else if ((*(tPVU32)(LMEMDDRTRAINVAR + 0x58) & 0xff) > Vrefdqsrange_best) {
+          Vrefdqsrange_best = (*(tPVU32)(LMEMDDRTRAINVAR + 0x58) & 0xff);
+          Vrefdqs_best = Vrefstarta;
+          Vref144Best = *(tPVU32)(APB_REG_ADDRESS144);
+        }
...
+      *(tPVU32)(LMEMDDRTRAINVAR + 0x5c) = Vrefdqsrange_best;
+      *(tPVU32)(APB_REG_ADDRESS28) = (*(tPVU32)(APB_REG_ADDRESS28) & 0xee00ee00) | VrefPN_array[Vrefdqs_best][0] | (VrefPN_array[Vrefdqs_best][1] << 16) | 0x80008000; //option2
+//#endif
+	  }
...
+
+#if defined DEFAULT_PRINTS
+	putc('\n'); putc('\r'); putc('B'); putc('e'); putc('s'); putc('t'); putc(' '); putc('R'); putc('a'); putc('n'); putc('g');putc('e');putc(' ');print_dec(Vrefdqsrange_best); putc('\n');
+
+	putc('V'); putc('r'); putc('e'); putc('f'); putc(' '); putc('i'); putc('d'); putc('x'); putc(' ');print_dec(Vrefdqs_best); putc('\n');
+	putc('V');  putc('r');  putc('e');  putc('f');  putc(' ');  putc('R');  putc('e'); putc('g'); putc('i'); putc('s');  putc('t'); putc('e'); putc('r'); putc(' ');
+	putc('2');  putc('0');  putc('C');  putc(' '); print_reg((*(tPVU32)(APB_REG_ADDRESS20C))); putc('\n');
+
+	putc('V');  putc('r');  putc('e');  putc('f');  putc(' ');  putc('R');  putc('e'); putc('g'); putc('i'); putc('s');  putc('t'); putc('e'); putc('r'); putc(' ');
+	putc('2');  putc('1');  putc('0');  putc(' '); print_reg((*(tPVU32)(APB_REG_ADDRESS210))); putc('\n');
+
+	putc('D');  putc('Q');  putc('S');  putc(' ');  putc('D');  putc('e');  putc('l'); putc('a'); putc('y'); 
+	putc(' ');  putc('R'); putc('e'); putc('g');putc('i'); putc('s');  putc('t'); putc('e'); putc('r'); putc(' '); putc('1'); putc('4');  putc('4'); putc(' ');
+	print_reg(Vref144Best); putc('\n');
+#endif
+
+#if defined (DEFAULT_PRINTS_BOOTLOADER)
...
+
+#if defined DEFAULT_PRINTS
+	  putc('!');  putc('!');  putc('!');  putc(' ');putc('B'); putc('e');  putc('s');  putc('t');  putc(' ');  putc('R');  putc('a'); putc('n'); putc('g'); putc('e'); putc(' '); 
+	  putc('A');  putc('f');  putc('t'); putc('e'); putc('r');putc(' '); putc('R');  putc('e');  putc('a');  putc('d');  putc(' ');  putc('V'); putc('r'); putc('e');  putc('f');  putc(' '); 
+	  putc('T');  putc('r');  putc('a');  putc('i');putc('n'); putc('i');  putc('n');  putc('g');  putc(' ');  
+	  putc('('); putc('R'); putc('e');  putc('a');  putc('d');  putc(' ');  putc('D');  putc('Q'); putc(' '); putc('e');  putc('y');  putc('e'); putc(')');  putc(' ');  putc(':'); 
+	  putc('('); print_dec(Vrefdqsrange_best);  putc(' ');  putc('/');  putc('6'); putc('4'); putc(')');  putc(' ');  putc('*');  putc(' ');
+
+#if defined (DDR_75_125MHZ)
+	  putc('1'); putc('0');  putc('.');  putc('0');
+#endif
+#if defined (DDR_200_250MHZ)
+	  putc('5'); putc('.');  putc('0');
+#endif
...
+#endif
+#if defined (DDR_800MHZ)
+	  putc('1'); putc('.');  putc('2'); putc('5');
+#endif
+#if defined (DDR_1066MHZ)
+	  putc('1'); putc('.');  putc('0');
+#endif
+#if defined (DDR_1333MHZ)
+	  putc('0'); putc('.');  putc('7'); putc('6');  putc('9');
+#endif
+#if defined (DDR_1600MHZ)
+	  putc('0'); putc('.');  putc('6'); putc('2');  putc('5');
+#endif
+
+	  putc(')'); putc(' ');  putc('n');  putc('s');  putc(' '); putc('!');  putc('!');  putc('!'); putc('\n'); putc('\r');
+#endif
+
+#if defined (HW_ENVPRN)
+#if defined (DDR_75_125MHZ)
...
+	if (Vrefdqsrange_best > 10) {
+#if defined DEFAULT_PRINTS
+	putc('R'); putc('a'); putc('n'); putc('g');putc('e');putc(' ');print_dec(Vrefdqsrange_best); putc('\n');
+	putc('V');  putc('r');  putc('e');  putc('f');  putc(' ');  putc('R');  putc('e'); putc('g'); putc('i'); putc('s');  putc('t'); putc('e'); putc('r'); putc(' '); putc('2');  putc('0');  putc('C');  putc(' '); print_reg((*(tPVU32)(APB_REG_ADDRESS20C))); putc('\n');
+	putc('V');  putc('r');  putc('e');  putc('f');  putc(' ');  putc('R');  putc('e'); putc('g'); putc('i'); putc('s');  putc('t'); putc('e'); putc('r'); putc(' '); putc('2');  putc('1');  putc('0');  putc(' '); print_reg((*(tPVU32)(APB_REG_ADDRESS210))); putc('\n');
+	putc('D');  putc('Q');  putc('S');  putc(' ');  putc('D');  putc('e');  putc('l'); putc('a'); putc('y'); putc(' ');  putc('R'); putc('e'); putc('g'); putc('i'); putc('s');  putc('t'); putc('e'); putc('r'); putc(' '); putc('1'); putc('4');  putc('4'); putc(' '); print_reg(Vref144Best); putc('\n');
+#endif
+#if defined (HW_ENVPRN) || defined (DEFAULT_PRINTS_BOOTLOADER)
...
+#endif
+#if defined DEFAULT_PRINTS
+	  putc('!');  putc('!');  putc('!');  putc(' ');putc('B'); putc('e');  putc('s');  putc('t');  putc(' ');  putc('R');  putc('a'); putc('n'); putc('g');  putc('e');  putc(' '); putc('('); 
+	  putc('R'); putc('e');  putc('a');  putc('d');  putc(' ');  putc('D');  putc('Q'); putc(' '); putc('e');  putc('y');  putc('e'); putc(')');  putc(' ');  
+	  putc('('); print_dec(Vrefdqsrange_best);  putc(' ');  putc('/');  putc('6'); putc('4'); putc(')');  putc(' ');  putc('*');  putc(' ');
+
+#if defined (DDR_75_125MHZ)
+	  putc('1'); putc('0');  putc('.');  putc('0'); 
...
+  ddrsize_calc(ddr_type, ddr_freq);
+
+#if defined (CONFIG_SPX_FEATURE_GLOBAL_MEMORY_ECC_ENABLE)
+  *(tPVU32)(MC_REG_ADDRESS58) = CONFIG_SPX_FEATURE_GLOBAL_ECC_MEMORY_LOWER_ADDRESS;
+  *(tPVU32)(MC_REG_ADDRESS5C) = CONFIG_SPX_FEATURE_GLOBAL_ECC_MEMORY_UPPER_ADDRESS;
+  *(tPVU32)(MC_REG_ADDRESS08) = (*(tPVU32)(MC_REG_ADDRESS08) & 0xffffff00) | 0x00000067;
+  *(tPVU32)(MC_REG_ADDRESS60) = CONFIG_SPX_FEATURE_GLOBAL_ECC_MEMORY_LOWER_ADDRESS;
+  *(tPVU32)(MC_REG_ADDRESS64) = CONFIG_SPX_FEATURE_GLOBAL_ECC_MEMORY_UPPER_ADDRESS;  
...
+  *(tPVU32)(LMEMDDRTRAINVAR + 0x60) = READ_REG32(0x80000000);
+ 
+  *(tPVU32)(0x80000004) = 0xdeadbeef;
+  *(tPVU32)(LMEMDDRTRAINVAR + 0x64) = READ_REG32(0x80000004);
+
+#if defined (HW_ENV)
...
+#endif
+
+  *(tPVU32)(MC_REG_ADDRESS00) = (*(tPVU32)(MC_REG_ADDRESS00) & ~0x00000300) | 0x00000200; //Made ZQ Long to 0 and r_memenable set to 1
+if (p4_rev != 0x40) {
+  if (ddr_type == 1) {
+    // DDR4
+    // r_zq2anycounter=62 (16,15,14,13,12,11,10,9,8),
+    *(tPVU32)(MC_REG_ADDRESS04) = (*(tPVU32)(MC_REG_ADDRESS04) & ~0x0001ff00) | 0x00003e00; //tZQCS
+  }
+  else {
+    // DDR3
+    // r_zq2anycounter=30 (16,15,14,13,12,11,10,9,8),
+    *(tPVU32)(MC_REG_ADDRESS04) = (*(tPVU32)(MC_REG_ADDRESS04) & ~0x0001ff00) | 0x00001e00; //tZQCS
+  }  
+}  
+ *(tPVU32)(MC_REG_ADDRESS48) = 0x0;
+ *(tPVU32)(MC_REG_ADDRESS54) = 0x0;
+ *(tPVU32)(MC_REG_ADDRESS70) = 0x0;  
+
+
+  DELAY(5);
+
+  //shutoff mcss register clk
+  *(tPVU32)(SYS_MCSS_CLK_CTRL) = 0xA000; //Clock to debug bus is disabled to save power.
+
+  return 0;
+
+}//ddrinit
...
+#endif
+
+	get_arrays(ddr_type, &trc_arrays);
+
+	trc_array = trc_arrays[0];
+	tmrs2nmrs_array = trc_arrays[1];
+	tfawtras_array = trc_arrays[2];
+
+	for(i=0;i<4;i++)
+	{
+#if defined (DDR_1600MHZ)
+		array1[i] = array(ddr_type, 1, 0, i);	
+		array2[i] = array(ddr_type, 0, 0, i);	
+#endif
+#if defined (DDR_1333MHZ)
+		array1[i] = array(ddr_type, 1, 1, i);	
+		array2[i] = array(ddr_type, 0, 1, i);	
+#endif
+#if defined (DDR_1066MHZ)
+		array1[i] = array(ddr_type, 1, 2, i);	
+		array2[i] = array(ddr_type, 0, 2, i);	
+#endif
+#if defined (DDR_800MHZ)
+		array1[i] = array(ddr_type, 1, 3, i);	
+		array2[i] = array(ddr_type, 0, 3, i);	
+#endif
+#if defined (DDR_500MHZ)
+		array1[i] = array(ddr_type, 1, 4, i);	
+		array2[i] = array(ddr_type, 0, 4, i);	
+#endif
+#if defined (DDR_350_400MHZ)
+		array1[i] = array(ddr_type, 1, 5, i);	
+		array2[i] = array(ddr_type, 0, 5, i);	
+#endif
+#if defined (DDR_200_250MHZ)
+		array1[i] = array(ddr_type, 1, 6, i);	
+		array2[i] = array(ddr_type, 0, 6, i);	
+#endif
+#if defined (DDR_75_125MHZ)
+		array1[i] = array(ddr_type, 1, 7, i);	
+		array2[i] = array(ddr_type, 0, 7, i);	
+#endif
+	}
+
+  if (ddr_type == 1) {
...
+    *(tPVU32)(MC_REG_ADDRESS08) = (*(tPVU32)(MC_REG_ADDRESS08) & ~0x0000f000) | tmrs2nmrs_array << 12;
+    // tRC = 50. r_act2actsbcounter (24,23,22,21,20).
+    //*(tPVU32)(MC_REG_ADDRESS14) = (*(tPVU32)(MC_REG_ADDRESS14) & ~0x01f00000) | trc_array[ddr_freq+8] << 20;
+    *(tPVU32)(MC_REG_ADDRESS14) = (*(tPVU32)(MC_REG_ADDRESS14) & ~0x01f00000) | trc_array << 20;
...
+    *(tPVU32)(MC_REG_ADDRESS10) = (*(tPVU32)(MC_REG_ADDRESS10) & ~0x3ff00000) | tfawtras_array << 20;
+    // tRFC = 350. r_trfcplus10counter (7,6,5,4,3,2,1,0),
+    //*(tPVU32)(MC_REG_ADDRESS04) = (*(tPVU32)(MC_REG_ADDRESS04) & ~0x000000ff) | trfc10_array[ddr_freq+8][0]; 
+//    *(tPVU32)(MC_REG_ADDRESS04) = (*(tPVU32)(MC_REG_ADDRESS04) & ~0x000000ff) | array(1, (ddr_freq+8), 0); 
+    *(tPVU32)(MC_REG_ADDRESS04) = (*(tPVU32)(MC_REG_ADDRESS04) & ~0x000000ff) | array1[0]; 
+    // r_ref2actcounter (24,23,22,21,20,19,18,17), 
+//    *(tPVU32)(MC_REG_ADDRESS04) = (*(tPVU32)(MC_REG_ADDRESS04) & ~0x01fe0000) | trfc_array[ddr_freq+8][0] << 17; 
+//    *(tPVU32)(MC_REG_ADDRESS04) = (*(tPVU32)(MC_REG_ADDRESS04) & ~0x01fe0000) | array(0, (ddr_freq+8), 0) << 17; 
+    *(tPVU32)(MC_REG_ADDRESS04) = (*(tPVU32)(MC_REG_ADDRESS04) & ~0x01fe0000) | array2[0] << 17; 
+
+if (WL11RL12==1) //#if defined (WL11RL12)
+{
...
+    *(tPVU32)(MC_REG_ADDRESS00) = (*(tPVU32)(MC_REG_ADDRESS00) & ~0xffffffc7) | 0x02005404; // RZQ/2 Dynamic ODT.
+    while(((*(tPVU32)(MC_REG_ADDRESS00)) & 0x1000) != 0);
+} else {//#else
+    // MRS2. WL11RL12.
+    //*(tPVU32)(MC_REG_ADDRESS00) = (*(tPVU32)(MC_REG_ADDRESS00) & ~0xffffffc7) | 0x00105404;
+    *(tPVU32)(MC_REG_ADDRESS00) = (*(tPVU32)(MC_REG_ADDRESS00) & ~0xffffffc7) | 0x02105404; // RZQ/2 Dynamic ODT.
+    while(((*(tPVU32)(MC_REG_ADDRESS00)) & 0x1000) != 0);
+}//#endif
+if(DLL_OFF==1) {//#if defined (DLL_OFF)
+    // DLL OFF. MRS1.
...
+} else {//#else 
+    // MRS1.
+	*(tPVU32)(MC_REG_ADDRESS00) = (*(tPVU32)(MC_REG_ADDRESS00) & ~0xffffffc7) | 0x01013404; //60.
+    while(((*(tPVU32)(MC_REG_ADDRESS00)) & 0x1000) != 0); 
+    // MRS0.
+if (WL9RL10==1) {//#if defined (WL9RL10)
...
+    //*(tPVU32)(MC_REG_ADDRESS04) = (*(tPVU32)(MC_REG_ADDRESS04) & ~0x000000ff) | array(1, ddr_freq, 3); 
+    *(tPVU32)(MC_REG_ADDRESS04) = (*(tPVU32)(MC_REG_ADDRESS04) & ~0x000000ff) | array1[3]; 
+    // r_ref2actcounter (24,23,22,21,20,19,18,17), 
+//    *(tPVU32)(MC_REG_ADDRESS04) = (*(tPVU32)(MC_REG_ADDRESS04) & ~0x01fe0000) | trfc_array[ddr_freq][3] << 17; 
+//    *(tPVU32)(MC_REG_ADDRESS04) = (*(tPVU32)(MC_REG_ADDRESS04) & ~0x01fe0000) | array(0, ddr_freq, 3) << 17; 
+    *(tPVU32)(MC_REG_ADDRESS04) = (*(tPVU32)(MC_REG_ADDRESS04) & ~0x01fe0000) | array2[3] << 17; 
+
+    // r_zq2anycounter=254 (16,15,14,13,12,11,10,9,8), 
...
+    while(((*(tPVU32)(MC_REG_ADDRESS18)) & 0x20000) != 0);
+    *(tPVU32)(LMEMDDRTRAINVAR + 0x00) = (*(tPVU32)(MC_REG_ADDRESS10));
...
+    }
+  }
+  *(tPVU32)(LMEMDDRTRAINVAR + 0x00) = dqsgate_min;
+
+#if defined (HW_ENVPRN)
...
+    }
+  }
+  *(tPVU32)(LMEMDDRTRAINVAR + 0x04) = dqsgate_max;  
+#if defined (HW_ENVPRN)
...
+    }
+
+    *(tPVU32)(LMEMDDRTRAINVAR + 0x08) = dqsgate_min_bin;
+    *(tPVU32)(LMEMDDRTRAINVAR + 0x0c) = dqsgate_max_bin;  
+    // Average of min and max.
+    dqsgate_avg = dqsgate_min_bin + (dqsgate_max_bin + 1 - dqsgate_min_bin)/2;
+    *(tPVU32)(LMEMDDRTRAINVAR + 0x10) = dqsgate_avg;
+#if defined MORE_PRINT
+	/*More print options for gate training*/
+	 putc('\n');putc('\r');
+	 putc('D'); putc('Q'); putc('S'); putc(' '); putc('G'); putc('A'); putc('T');putc('E'); putc(' ');
+	 putc('M'); putc('I'); putc('N'); putc(' '); putc('B'); putc('y'); putc('t');putc('e'); putc('0');
+	 putc(' '); putc('b'); putc('i'); putc('n'); putc(' '); putc('V'); putc('a');putc('l'); putc('u'); putc('e');putc(' '); print_reg(dqsgate_min_bin);putc(' ');
+	 putc('D'); putc('Q'); putc('S'); putc(' '); putc('G'); putc('A'); putc('T');putc('E'); putc(' ');
+	 putc('M'); putc('A'); putc('X'); putc(' ');  putc(' '); putc('b'); putc('i'); putc('n'); putc(' '); putc('V'); putc('a');putc('l'); putc('u'); putc('e');putc(' '); print_reg(dqsgate_max_bin);putc(' ');
+	 putc('D'); putc('Q'); putc('S'); putc(' '); putc('G'); putc('A'); putc('T');putc('E'); putc(' ');
+	 putc('A'); putc('V'); putc('G'); putc(' ');  putc(' '); putc('b'); putc('i'); putc('n'); putc(' '); putc('V'); putc('a');putc('l'); putc('u'); putc('e');putc(' '); print_reg(dqsgate_avg);putc('\n');
+#endif
+#if defined (HW_ENVPRN)
...
+    }
+  }
+  *(tPVU32)(LMEMDDRTRAINVAR + 0x14) = dqsgate_min;
+#if defined (HW_ENVPRN)
...
+    }
+  }
+  *(tPVU32)(LMEMDDRTRAINVAR + 0x18) = dqsgate_max;
+#if defined (HW_ENVPRN)
...
+    }
+
+    *(tPVU32)(LMEMDDRTRAINVAR + 0x1c) = dqsgate_min_bin;
+    *(tPVU32)(LMEMDDRTRAINVAR + 0x20) = dqsgate_max_bin;
+    // Average of min and max.
+    dqsgate_avg = dqsgate_min_bin + (dqsgate_max_bin + 1 - dqsgate_min_bin)/2;
+    *(tPVU32)(LMEMDDRTRAINVAR + 0x24) = dqsgate_avg;
+#if defined MORE_PRINT
+	/*More print options for gate training*/
+	 putc('D'); putc('Q'); putc('S'); putc(' '); putc('G'); putc('A'); putc('T');putc('E'); putc(' ');
+	 putc('M'); putc('I'); putc('N'); putc(' '); putc('B'); putc('y'); putc('t');putc('e'); putc('1');
+	 putc(' '); putc('b'); putc('i'); putc('n'); putc(' '); putc('V'); putc('a');putc('l'); putc('u'); putc('e');putc(' '); print_reg(dqsgate_min_bin);putc(' ');
+	 putc('D'); putc('Q'); putc('S'); putc(' '); putc('G'); putc('A'); putc('T');putc('E'); putc(' ');
+	 putc('M'); putc('A'); putc('X'); putc(' ');  putc(' '); putc('b'); putc('i'); putc('n'); putc(' '); putc('V'); putc('a');putc('l'); putc('u'); putc('e');putc(' '); 
+	 print_reg(dqsgate_max_bin);putc(' ');
+	 putc('D'); putc('Q'); putc('S'); putc(' '); putc('G'); putc('A'); putc('T');putc('E'); putc(' ');
+	 putc('A'); putc('V'); putc('G'); putc(' ');  putc(' '); putc('b'); putc('i'); putc('n'); putc(' '); putc('V'); putc('a');putc('l'); putc('u'); putc('e');putc(' '); 
+	 print_reg(dqsgate_avg);putc('\n');
+#endif
+#if defined (HW_ENVPRN)
...
+	 putc('\n'); putc('\r');
+	 putc('D'); putc('Q'); putc('S'); putc(' '); putc('G'); putc('A'); putc('T');putc('E'); putc(' ');
+	 putc('R'); putc('e'); putc('g'); putc('i'); putc('s');  putc('t'); putc('e'); putc('r');putc(' '); putc('3'); putc('4'); putc(' ');
+	 putc('B'); putc('y'); putc('t'); putc('e'); putc('1'); putc(' '); putc('B');putc('y'); putc('t');
+	 putc('e'); putc('0'); putc(' '); putc('V'); putc('a');;putc('l'); putc('u'); putc('e');putc(' '); 
+	 print_reg((*(tPVU32)(APB_REG_ADDRESS34))); putc('\n');
+#endif
+#if defined (HW_ENV)
+#if defined (HW_ENVPRN) || defined (DEFAULT_PRINTS_BOOTLOADER)
...
+#endif
+  }
+  *(tPVU32)(LMEMDDRTRAINVAR + 0x28) = dqsvalue_min;
+#if defined (HW_ENVPRN)
...
+#endif
+  }
+  *(tPVU32)(LMEMDDRTRAINVAR + 0x2c) = dqsvalue_max;  
+#if defined (HW_ENVPRN)
...
+  }
+  *(tPVU32)(LMEMDDRTRAINVAR + 0x30) = dqsvalue_min_bin;
+  *(tPVU32)(LMEMDDRTRAINVAR + 0x34) = dqsvalue_max_bin; 
+  // Average of min and max.
+  dqsvalue_avg = dqsvalue_min_bin + (dqsvalue_max_bin + 1 - dqsvalue_min_bin)/2;
+  *(tPVU32)(LMEMDDRTRAINVAR + 0x38) = dqsvalue_avg;
+#if defined MORE_PRINT
+  /*more print options for DQS Delay Training values*/
+	 putc('D'); putc('Q'); putc('S'); putc(' '); putc('E'); putc('Y'); putc('E');putc(' ');  putc('B'); putc('y'); putc('t');putc('e');  putc('0'); putc(' ');
+	 putc('M'); putc('I'); putc('N'); putc(' '); putc('b'); putc('i'); putc('n');putc(' ');  print_reg(dqsvalue_min_bin);putc(' ');
+	 putc('M'); putc('A'); putc('X'); putc(' '); putc('b'); putc('i'); putc('n');putc(' ');  print_reg(dqsvalue_max_bin);putc(' ');
+	 putc('A'); putc('V'); putc('G'); putc(' '); putc('b'); putc('i'); putc('n');putc(' ');  print_reg(dqsvalue_avg);putc('\n');
+#endif
+#if defined (HW_ENVPRN)
...
+  *(tPVU32)(LMEMDDRTRAINVAR + 0x3c) = range;
+
+  // Second Byte.
+#if defined (STARTFROMCENTERSWEEP_OFF)
+  gindex = 0;
...
+#endif
+  }
+  *(tPVU32)(LMEMDDRTRAINVAR + 0x40) = dqsvalue_min;
+#if defined (HW_ENVPRN)
...
+#endif
+  }
+  *(tPVU32)(LMEMDDRTRAINVAR + 0x44) = dqsvalue_max;  
+#if defined (HW_ENVPRN)
...
+     dqsvalue_max_bin = (dqsvalue & 0x1f) | (((~sbit) << 5) & 0x20);
+  }
+  *(tPVU32)(LMEMDDRTRAINVAR + 0x48) = dqsvalue_min_bin;
+  *(tPVU32)(LMEMDDRTRAINVAR + 0x4c) = dqsvalue_max_bin; 
+  // Average of min and max.
+  dqsvalue_avg = dqsvalue_min_bin + (dqsvalue_max_bin + 1 - dqsvalue_min_bin)/2;  
+  *(tPVU32)(LMEMDDRTRAINVAR + 0x50) = dqsvalue_avg;
+#if defined MORE_PRINT
+  /*More print options for DQS delay training values*/
+	 putc('D'); putc('Q'); putc('S'); putc(' '); putc('E'); putc('Y'); putc('E');putc(' ');  putc('B'); putc('y'); putc('t');putc('e');  putc('1'); putc(' ');
+	 putc('M'); putc('I'); putc('N'); putc(' '); putc('b'); putc('i'); putc('n');putc(' ');  print_reg(dqsvalue_min_bin);putc(' ');
+	 putc('M'); putc('A'); putc('X'); putc(' '); putc('b'); putc('i'); putc('n');putc(' ');  print_reg(dqsvalue_max_bin);putc(' ');
+	 putc('A'); putc('V'); putc('G'); putc(' '); putc('b'); putc('i'); putc('n');putc(' ');  print_reg(dqsvalue_avg);putc('\n');
+#endif
...
+#if defined DEFAULT_PRINTS
+	 putc('D'); putc('Q'); putc('S'); putc(' '); putc('G'); putc('A'); putc('T');putc('E'); putc(' ');
+	 putc('R'); putc('e'); putc('g');  putc('i');putc('s'); putc('t'); putc('e'); putc('r');putc(' '); putc('1'); putc('4'); putc('4'); putc(' ');
+	 putc('B'); putc('y'); putc('t'); putc('e'); putc('1'); putc(' '); putc('B');putc('y'); putc('t');
+	 putc('e'); putc('0'); putc(' '); putc('V'); putc('a');;putc('l'); putc('u'); putc('e');putc(' '); 
+	 print_reg((*(tPVU32)(APB_REG_ADDRESS144))); putc('\n');
+#endif
+#if defined (HW_ENVPRN)
...
+  }
+
+  *(tPVU32)(LMEMDDRTRAINVAR + 0x54) = (dqsvalue_max_bin + 1 - dqsvalue_min_bin);
+  if (range > (dqsvalue_max_bin + 1 - dqsvalue_min_bin)) {
+     range = (dqsvalue_max_bin + 1 - dqsvalue_min_bin);
+  }
+  *(tPVU32)(LMEMDDRTRAINVAR + 0x58) = range;
+ 
+  return not_found;
+} // eye_training
+
...
+#if defined (HW_ENV)
+#if defined DEFAULT_PRINTS
+	 putc('F'); putc('I'); putc('F'); putc('O');  putc(' '); putc('t'); putc('r'); putc('a'); putc('i'); putc('n');
+	 putc('i');;putc('n');  putc('g'); putc(' '); putc('v'); putc('a');;putc('l'); putc('u'); putc('e');putc(' '); 
+	 print_dec(fifovalue); putc('\n');
+#endif
+#if defined (DEFAULT_PRINTS_BOOTLOADER)
...
+void ddrsize_calc(tU8 ddr_type, tU8 freq_value)
+{
+  tU8 ddr_tech_type;
+  int i;
+	tVU8 array1[4];
...
+ #endif
+  //*(tPVU32)(MC_REG_ADDRESS04) = (*(tPVU32)(MC_REG_ADDRESS04) & ~0x000000ff) | array(1, (freq_value+8), (ddr_tech_type/2));
+  *(tPVU32)(MC_REG_ADDRESS04) = (*(tPVU32)(MC_REG_ADDRESS04) & ~0x000000ff) | array1[(ddr_tech_type/2)];
+  //*(tPVU32)(MC_REG_ADDRESS04) = (*(tPVU32)(MC_REG_ADDRESS04) & ~0x01fe0000) | array(0, (freq_value+8), (ddr_tech_type/2)) << 17;
+  *(tPVU32)(MC_REG_ADDRESS04) = (*(tPVU32)(MC_REG_ADDRESS04) & ~0x01fe0000) | array2[(ddr_tech_type/2)] << 17;
+} else {
+//#else
+  *(tPVU32)(MC_REG_ADDRESS04) = (*(tPVU32)(MC_REG_ADDRESS04) & 0xf1ffffff) | ddr_tech_type << 25;
+#if 0
...
+ #endif
+  //*(tPVU32)(MC_REG_ADDRESS04) = (*(tPVU32)(MC_REG_ADDRESS04) & ~0x000000ff) | array(1, (freq_value), (ddr_tech_type/2));
+  *(tPVU32)(MC_REG_ADDRESS04) = (*(tPVU32)(MC_REG_ADDRESS04) & ~0x000000ff) | array1[(ddr_tech_type/2)];
+//  *(tPVU32)(MC_REG_ADDRESS04) = (*(tPVU32)(MC_REG_ADDRESS04) & ~0x01fe0000) | array(0, (freq_value), (ddr_tech_type/2)) << 17;
+  *(tPVU32)(MC_REG_ADDRESS04) = (*(tPVU32)(MC_REG_ADDRESS04) & ~0x01fe0000) | array2[(ddr_tech_type/2)] << 17;
+ }
+//#endif
+
