...
static long bt_ioctlUnlocked (struct file *file, unsigned int cmd, unsigned long arg);
#else
...
static struct file_operations bt_fops = 
{
    ...
    .write 		= bt_write,
#ifdef USE_UNLOCKED_IOCTL 
    .unlocked_ioctl	=	bt_ioctlUnlocked, 
#else 
    .ioctl		= bt_ioctl,
#endif 
    .open  		= bt_open,
...
static char banner[] __initdata = KERN_INFO "BT Common Driver, (c) 2009-2015 American Megatrends Inc...\n";	
...
{
	struct bt_hal *pbt_hal = NULL;
	unsigned int minor = iminor(inode);
	struct bt_dev* pdev = NULL;
	BTBuf_T* pBTBuffer = NULL;
...
static long 
bt_ioctlUnlocked (struct file *file, unsigned int cmd, unsigned long arg)
{
	int ret = 0;
	struct bt_dev *pdev = (struct bt_dev*) file->private_data;
...
{
	return (bt_ioctlUnlocked (file, cmd, arg));
}
#endif
static int 
...
int bt_core_loaded =1;
EXPORT_SYMBOL(bt_core_loaded);
