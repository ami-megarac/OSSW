--- linux-3.14.17/drivers/net/ethernet/pilot_tulip/media.c	2016-06-23 15:51:32.687531992 +0800
+++ linux-3.14.17.new/drivers/net/ethernet/pilot_tulip/media.c	2016-06-23 15:03:35.544033992 +0800
@@ -0,0 +1,669 @@
+        /*
...
+        #endif 
+	#ifdef CONFIG_SPX_FEATURE_GLOBAL_SHARED_MDIO_MDC_LINES
+		spin_lock_irqsave(&miilock, flags);
+	#else
...
+	#ifdef CONFIG_SPX_FEATURE_GLOBAL_SHARED_MDIO_MDC_LINES
+                        spin_unlock_irqrestore(&miilock, flags);
+	#else
...
+
+        struct tulip_private *tp = netdev_priv(dev);
+        unsigned org_csr6 = tp->csr6;
+        int retval;
+        int LinkChange=0;
+
+        retval = pilot_up(dev,0);
+        switch (retval)
+        {
+                case -2:                /* No PHY */
+                        tp->linkdown = 0;
...
+        phyid = tulip_mdio_read(dev, phy, MII_PHYSID1);
+#else
...
+	tp->phy_oui = (mii_status & 0xFFFF) << 6; /* OUI[21:6] in MII_PHYSID1[15:0] */
+
+	mii_status = tulip_mdio_read(dev, phy, MII_PHYSID2);
+	tp->phy_oui |= (mii_status & 0xFFFF) >> 10; /* OUI[5:0] in MII_PHYSID2[15:10] */
+
+	tp->phy_model = (mii_status & 0x03F0) >> 4; /* MODEL[5:0] in MII_PHYSID2[9:4] */
+
+	tp->phy_rev = (mii_status & 0x000F); /* Revision[5:0] in MII_PHYSID2[3:0] */
+
+        mii_status = tulip_mdio_read (dev, phy, MII_BMSR);
...
+                        continue;
+                }
+                tulip_init_mii(dev,board_idx,phy_idx,phy);
+                phy_idx++;
+
