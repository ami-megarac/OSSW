--- linux-3.14.17/drivers/net/ethernet/pilot_tulip/tulip.h	2016-08-16 17:51:30.171712131 +0800
+++ linux-3.14.17.new/drivers/net/ethernet/pilot_tulip/tulip.h	2016-08-16 18:02:06.133248143 +0800
@@ -0,0 +1,557 @@
+/*
...
+};
+
+#define MAC_TT    (0xf<<27)
+#define MAC_TIM   (0x1ffff)
+#define MAC_TIM_1 (0x10fff)    
+#define MAC_RT    (0xf<<20)
+#define MAC_NTP (0x7<<24)
+
+#define CSR11_TIM       (MAC_TIM|MAC_RT)  //NORMAL GTE
+#define CSR11_TIM_1    (MAC_TIM_1|MAC_RT)  //reduced GTE
+#define CSR11_NTP      (MAC_TIM|MAC_RT|MAC_NTP) //enable NTP also
+
+
+
+enum MediaIs {
...
+
+		if (!i)
+			printk(KERN_DEBUG "tulip_stop_rxtx() failed"
+					" (CSR5 0x%x CSR6 0x%x)\n",
