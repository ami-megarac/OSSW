...
static int peci_irq;
static DECLARE_COMPLETION(ast_peci_done);
		...
		for (i = 0; i < 4; i ++) {
			reg |= ((uint32_t) buf[(4 * reg_num) + i]) << (8 * i);
                ...
                if (reg_num < 4){
                        ...
                        ast_peci_write_reg(reg, AST_PECI_W_DATA_4 + (4 * (reg_num % 4))); /* data */
                }
...
static void ast_peci_read_data(unsigned char *rx_buf, unsigned int rx_len)
{
	...
	while (rx_len > 0) {
                if (reg_num < 4 ){
                        ...
                        reg = ast_peci_read_reg(AST_PECI_R_DATA_4 + (4 * (reg_num % 4)));
                }
		for (i = 0; i < 4; i ++) {
...
static void ast_peci_init_hw(void)
{
	...
	reg |= AST_PECI_CTRL_ENABLE_CLK |
			((AST_PECI_CTRL_CLK_1 << AST_PECI_CTRL_CLK_SHIFT) & AST_PECI_CTRL_CLK_MASK) |
...
static int ast_peci_send_cmd(peci_cmd_t *cmd)
{
	...
	reinit_completion(&ast_peci_done);
...
static int __init ast_peci_module_init(void)
{
	...
	peci_irq = GetIrqFromDT("ami_peci", AST_PECI_IRQ);
	ret = request_irq(peci_irq, ast_peci_irq_handler, IRQF_SHARED, AST_PECI_DRIVER_NAME, ast_peci_virt_base);
	if (ret) {
...
out_irq:
	free_irq(peci_irq, ast_peci_virt_base);
out_iomap:
