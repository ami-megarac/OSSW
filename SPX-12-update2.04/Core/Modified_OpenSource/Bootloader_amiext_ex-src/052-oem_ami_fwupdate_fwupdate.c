--- uboot_old/oem/ami/fwupdate/fwupdate.c	1970-01-01 05:30:00.000000000 +0530
+++ uboot/oem/ami/fwupdate/fwupdate.c	2017-08-16 15:11:53.769027601 +0530
@@ -0,0 +1,685 @@
+/*****************************************************************
+ *****************************************************************
+ ***                                                            **
...
+			pFlashInfo = &flash_info[bank];
+
+			if (0 != flash_read (pFlashInfo->start[SectorToUpdate], CONFIG_SPX_FEATURE_GLOBAL_ERASE_BLOCK_SIZE, (char *)TempBuf))
+			{
...
+			while (retries)
+			{
+				if (0 != flash_erase (pFlashInfo, SectorToUpdate, SectorToUpdate))
+				{
...
+							
+			src   = pRamAddress;
+			addr = pFlashInfo->start[SectorToUpdate];
+			cnt  = CONFIG_SPX_FEATURE_GLOBAL_ERASE_BLOCK_SIZE;
+			retries = 3;
+
+			while (retries)
...
+					retries--;
+					flash_erase (pFlashInfo, SectorToUpdate, SectorToUpdate);
+					continue;
+				}	
+				break;
+			}
