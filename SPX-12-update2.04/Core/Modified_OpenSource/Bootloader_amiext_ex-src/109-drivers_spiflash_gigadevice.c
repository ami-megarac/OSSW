--- uboot.old/drivers/spiflash/gigadevice.c	1970-01-01 08:00:00.000000000 +0800
+++ uboot/drivers/spiflash/gigadevice.c	2019-10-24 18:56:47.784768651 +0800
@@ -0,0 +1,199 @@
+/*
...
+	cmd_size = 1;
+	retval = ctrl_drv->spi_transfer(bank,command, cmd_size ,SPI_READ, &reg_data, 1, 0);
+	if (retval < 0)
+	{
...
+{
+	init_MUTEX(&gigadevice_driver.lock);
