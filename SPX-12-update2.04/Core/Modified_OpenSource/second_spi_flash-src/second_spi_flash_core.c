...
#define SECOND_SPI_FLASH_MAJOR_NUM            154
#define SECOND_SPI_FLASH_MINOR_NUM            0
#define SECOND_SPI_FLASH_MAX_DEVICES          1
static dev_t second_spi_flash_devno = MKDEV(SECOND_SPI_FLASH_MAJOR_NUM, SECOND_SPI_FLASH_MINOR_NUM);
static struct cdev *second_spi_flash_cdev;

#ifdef USE_UNLOCKED_IOCTL
static long second_spi_flash_module_ioctlUnlocked(struct file *file, uint cmd, ulong arg);
#else
static int second_spi_flash_module_ioctl(struct inode *inode, struct file *file, uint cmd, ulong arg);
#endif
static int second_spi_flash_open(struct inode *inode, struct file *file);
static int second_spi_flash_release(struct inode *inode, struct file *file);
static int second_spi_flash_reset(struct mtd_info *mtd);

#define SECOND_SPI_FLASH_DRIVER_NAME          "second_spi_flash"
#define ADDR_16MB                           0x1000000

#define BYTE_3_ADDR_MODE                    0
#define BYTE_4_ADDR_MODE                    1

struct second_spi_flash_info_t *g_spi_info = NULL;
struct semaphore second_spi_flash_lock;
struct second_spi_flash_core_ops_t second_spi_flash_core_ops;
static int spi_ctrl_4b_mode = 0;

struct second_spi_flash_info_t
{
    char        *name;
    uint32_t    id;
...
static struct file_operations second_spi_flash_ops = {
        owner:      THIS_MODULE,
        read:       NULL,
        write:      NULL,
#ifdef USE_UNLOCKED_IOCTL
        unlocked_ioctl: second_spi_flash_module_ioctlUnlocked,
#else
        ioctl:      second_spi_flash_module_ioctl,
#endif
        open:       second_spi_flash_open,
        release:    second_spi_flash_release,
};
static struct second_spi_flash_info_t second_spi_flash_info_tab[] =
{
...
static uint32_t read_id(struct second_spi_flash_hal_ops_t *hw_ops)
{
    unsigned char cmd;
    uint8_t ids[4];
    uint32_t id;
 ...
 */
static int wait_for_wip_status(struct second_spi_flash_hal_ops_t *hw_ops)
{
    unsigned char cmd;
    uint8_t status = 0;
    int count = 0;
...
static int second_spi_flash_reset(struct mtd_info *mtd)
{
        struct second_spi_flash_hal_ops_t *hw_ops;
        int retval;
        u8 code = 0;

        //ENABLE RESET
        code =  SPI_FLASH_CMD_RESET_ENABLE;
        hw_ops = mtd->priv;
...
static int is_wel_enabled(struct second_spi_flash_hal_ops_t *hw_ops)
{
    unsigned char cmd;
    uint8_t status = 0;
    int count = 0;
...
static void enable_write(struct second_spi_flash_hal_ops_t *hw_ops)
{
    unsigned char cmd;
...
int enter_4byte_addr_mode(struct second_spi_flash_hal_ops_t *hw_ops)
{
...
int exit_4byte_addr_mode(struct second_spi_flash_hal_ops_t *hw_ops)
{
...
static int get_erase_cmd(uint32_t erasesize, uint32_t *cmd)
{
    switch (erasesize)
    {
        ...
        case 0x10000:
        {
            if (spi_ctrl_4b_mode)
			{
				// 4 byte sector erase command not supported
				if (current_flash_id == 0x19BA20 || current_flash_id == 0x1940EF)
...
#if 0
/* It will be useful for some SPI to use 4byte address mode using extended address */
static void extended_address_write(struct second_spi_flash_hal_ops_t *hw_ops,unsigned char data)
{
    unsigned char cmd[2];
    cmd[0] = 0xC5;
    cmd[1] = data;
    ...
    enable_write(hw_ops);
    hw_ops->transfer(cmd, 1, &data, 1, 1);

    if (wait_for_wip_status(hw_ops))
    {
        printk("Error wait_for_wip_status \n");
    }
 ...
 */
static int erase_sector(struct second_spi_flash_hal_ops_t *hw_ops, uint32_t offset, uint32_t command)
{
...
static void write_buffer(struct second_spi_flash_hal_ops_t *hw_ops, const uint8_t *src, uint32_t offset, int len, uint8_t command)
{
    unsigned char cmd[5] = {0};
    int cmd_size = 4;
    int index = 0;
    ...
    {
		cmd[index++] = command;
        cmd[index++] = (unsigned char) (offset >> 24);
...
static int second_spi_flash_erase(struct mtd_info *mtd, struct erase_info *instr)
{
    struct second_spi_flash_hal_ops_t *hw_ops;
    uint32_t addr, len;
...
#endif
    uint32_t eraseblocksize = 0, command = 0;
	...
	/* Enter 4byte mode to access beyond 16MB, If 4byte is not supported for specific SPI or commands then 
		need to use extended address register to access the upper memory region */
	if ((spi_ctrl_4b_mode) && (command == SPI_FLASH_CMD_BLK_ERASE_64K))
	{
	...
	while (len)
    {
        if (erase_sector(hw_ops, addr,command))
        {
            instr->state = MTD_ERASE_FAILED;
            up(&second_spi_flash_lock);
            return -EIO;
        }
...
static int second_spi_flash_read(struct mtd_info *mtd, loff_t from, size_t len, size_t *retlen, u_char *buf)
{
    struct second_spi_flash_hal_ops_t *hw_ops;
    ...
    while (len > 0)
    {
		...
		// It is better to use 4 byte fast read command.
		if (current_flash_id == 0x19BA20 || current_flash_id == 0x1940EF)		
        ...
        if (spi_ctrl_4b_mode)
        {
			cmd[index++] = (unsigned char) (from >> 24);	
...
static int second_spi_flash_write(struct mtd_info *mtd, loff_t to, size_t len, size_t *retlen, const u_char *buf)
{
    struct second_spi_flash_hal_ops_t *hw_ops;
	...
	if(spi_ctrl_4b_mode)
	{
        // 4 byte page program command not supported
        if (current_flash_id == 0x19BA20 || current_flash_id == 0x1940EF)
		{
            command  = SPI_FLASH_CMD_PAGE_PROGRAM;
		    enter_4byte_addr_mode(hw_ops);
		}
        else
		{
            command = SPI_FLASH_CMD_PAGE_PROGRAM_4B_MODE;
		}
    }
    else
    {
    ...
    {
        write_buffer(hw_ops, buf, to, len, command);
        *retlen += len;
    }
    else     /* multi pages */
    {
        ...
        write_buffer(hw_ops, buf, to, page_size, command);
        *retlen += page_size;
        ...
        for (i = page_size; i < len; i += page_size)
        {
            ...
            write_buffer(hw_ops, buf + i, to + i, page_size, command);
            *retlen += page_size;
        }
    }

	if((spi_ctrl_4b_mode) && (command == SPI_FLASH_CMD_PAGE_PROGRAM))
	{
		exit_4byte_addr_mode(hw_ops);
	}
...
static struct mtd_info *second_spi_flash_probe(struct second_spi_flash_hal_ops_t *hw_ops)
{
    struct second_spi_flash_info_t *info;
    struct mtd_info *new_mtd;
    uint32_t id;
    int i;

    id = read_id(hw_ops);
    for (i = 0, info = second_spi_flash_info_tab; i < ARRAY_SIZE(second_spi_flash_info_tab); i ++, info ++)
    {
        if (id == info->id)
        {
            break;
    ...
    if (i == ARRAY_SIZE(second_spi_flash_info_tab))   /* no matched id */
    {
        printk(KERN_WARNING "%s: not supported id %06x\n", SECOND_SPI_FLASH_DRIVER_NAME, id);
        return NULL;
    }
...
#if (LINUX_VERSION_CODE >=  KERNEL_VERSION(3,4,11))
	new_mtd->_erase = second_spi_flash_erase;
	new_mtd->_read = second_spi_flash_read;
	new_mtd->_write = second_spi_flash_write;
#else
    new_mtd->erase = second_spi_flash_erase;
    new_mtd->read = second_spi_flash_read;
    new_mtd->write = Second_spi_flash_write;
#endif
    new_mtd->writesize = 1;
    current_flash_id = id;
    g_spi_info = info;
...
int second_spi_flash_register(unsigned char num, void *hw_ops, void **hw_data)
{
    struct second_spi_flash_hal_t *hw_hal;
    ...
    hw_hal = (struct second_spi_flash_hal_t *) kmalloc(sizeof(struct second_spi_flash_hal_t), GFP_KERNEL);
    if (hw_hal == NULL)
    {
        return -ENOMEM;
    }

    hw_hal->ops = (struct second_spi_flash_hal_ops_t *) hw_ops;
    hw_hal->mtd = second_spi_flash_probe(hw_hal->ops);
    if (hw_hal->mtd == NULL)
    {
        kfree(hw_hal);
        return -ENODEV;
    ...
    hw_hal->partitions.name = CONFIG_SPX_FEATURE_SECOND_SPI_FLASH_MTD_NAME;
...
int second_spi_flash_unregister(void *hw_data)
{
    struct second_spi_flash_hal_t *hw_hal = (struct second_spi_flash_hal_t *) hw_data;
...
static core_hal_t second_spi_flash_core_hal =
{
    .owner = THIS_MODULE,
    .name = "second SPI flash core",
    .dev_type = EDEV_TYPE_SECOND_SPI_FLASH,
    .register_hal_module = second_spi_flash_register,
    .unregister_hal_module = second_spi_flash_unregister,
    .pcore_funcs = (void *) &second_spi_flash_core_ops
};

int second_spi_flash_init(void)
{
    int ret;

    /* Second_spi_flash device initialization */
    if ((ret = register_chrdev_region (second_spi_flash_devno, SECOND_SPI_FLASH_MAX_DEVICES, SECOND_SPI_FLASH_DRIVER_NAME)) < 0)
    {
        printk (KERN_ERR "failed to register second_spi_flash device <%s> (err: %d)\n", SECOND_SPI_FLASH_DRIVER_NAME, ret);
        return -1;
    }

    second_spi_flash_cdev = cdev_alloc();
    if(!second_spi_flash_cdev)
    {
        unregister_chrdev_region (second_spi_flash_devno, SECOND_SPI_FLASH_MAX_DEVICES);
        printk (KERN_ERR "%s: failed to allocate second_spi_flash cdev structure\n", SECOND_SPI_FLASH_DRIVER_NAME);
        return -1;
    }
    cdev_init (second_spi_flash_cdev, &second_spi_flash_ops);
    second_spi_flash_cdev->owner = THIS_MODULE;

    if ((ret = cdev_add (second_spi_flash_cdev, second_spi_flash_devno, SECOND_SPI_FLASH_MAX_DEVICES)) < 0)
    {
        cdev_del (second_spi_flash_cdev);
        unregister_chrdev_region (second_spi_flash_devno, SECOND_SPI_FLASH_MAX_DEVICES);
    ...
    //init_MUTEX(&second_spi_flash_lock);
    sema_init(&second_spi_flash_lock, 1);

    ret = register_core_hal_module(&second_spi_flash_core_hal);
    if (ret < 0)
    {
        printk(KERN_ERR "failed to register the second SPI flash core module\n");
        cdev_del (second_spi_flash_cdev);
        unregister_chrdev_region (second_spi_flash_devno, SECOND_SPI_FLASH_MAX_DEVICES);
        ret = -EINVAL;
        return ret;
...
void __exit second_spi_flash_exit(void)
{
    unregister_core_hal_module(EDEV_TYPE_SECOND_SPI_FLASH);
    unregister_chrdev_region (second_spi_flash_devno, SECOND_SPI_FLASH_MAX_DEVICES);
    if (NULL != second_spi_flash_cdev)
    {
        cdev_del (second_spi_flash_cdev);
    }
}
static int 
second_spi_flash_open(struct inode *inode, struct file *file)
{
    unsigned int minor = iminor(inode);
    struct second_spi_flash_hal_t *psecond_spi_flash_hal;
    struct second_spi_flash_dev *pdev;
    hw_info_t second_spi_flash_hw_info;
    int ret;
    unsigned char open_count;

    ret = hw_open (EDEV_TYPE_SECOND_SPI_FLASH, minor, &open_count, &second_spi_flash_hw_info);
    if (ret)
        return -ENXIO;

    psecond_spi_flash_hal = second_spi_flash_hw_info.pdrv_data;

    pdev = (struct second_spi_flash_dev*)kmalloc(sizeof(struct second_spi_flash_dev), GFP_KERNEL);

    if (!pdev)
    {
        hw_close (EDEV_TYPE_SECOND_SPI_FLASH, minor, &open_count);
        printk (KERN_ERR "%s: failed to allocate private dev structure for second_spi_flash iminor: %d\n", SECOND_SPI_FLASH_DRIVER_NAME, minor);
        return -ENOMEM;
    }

    pdev->psecond_spi_flash_hal = psecond_spi_flash_hal;
...
static int
second_spi_flash_release(struct inode *inode, struct file *file)
{
    int ret;
    unsigned char open_count;
    struct second_spi_flash_dev *pdev = (struct second_spi_flash_dev*)file->private_data;
    pdev->psecond_spi_flash_hal = NULL;
    file->private_data = NULL;
    ret = hw_close (EDEV_TYPE_SECOND_SPI_FLASH, iminor(inode), &open_count);
    if(ret) { return -1; }
        kfree (pdev);
    return 0;
}
...
static long
second_spi_flash_module_ioctlUnlocked(struct file *file, uint cmd, ulong arg)
{
   	struct second_spi_flash_dev *pdev = (struct second_spi_flash_dev*) file->private_data;
   	int second_spi_access = 0;

    switch (cmd)
    {
         case SET_SECOND_SPI_ACCESS:
            if(copy_from_user((void *)&second_spi_access,(void *)arg, sizeof(int)))
            {
                printk("SET_SECOND_SPI_ACCESS: Error copying data from user \n");
                return -EFAULT;
            }
            if (pdev->psecond_spi_flash_hal->ops->second_spi_flash_change_mode)
                pdev->psecond_spi_flash_hal->ops->second_spi_flash_change_mode(second_spi_access);
            break;

         case RESET_SPI:
        	 second_spi_flash_reset(pdev->psecond_spi_flash_hal->mtd);
        	 break;

         default:
             printk("ERROR: second_spi_flash: Invalid IOCTL \n");
             return -EINVAL;
   	}
    return 0;
}
...
static int
second_spi_flash_module_ioctl(struct inode *inode, struct file *file, uint cmd, ulong arg)
{
        return (second_spi_flash_module_ioctlUnlocked(file, cmd, arg));
}
#endif

module_init(second_spi_flash_init);
module_exit(second_spi_flash_exit);

MODULE_AUTHOR("American Megatrends Inc.");
MODULE_DESCRIPTION("Common module of second SPI flash driver");
MODULE_LICENSE("GPL");
