...
i2c_as_init( void )
{
	...
	dt_irq = GetIrqFromDT("ami_i2c", IRQ_I2C);
#if (LINUX_VERSION_CODE >=  KERNEL_VERSION(2,6,24)) 
    if( request_irq( dt_irq, i2c_handler, IRQF_SHARED, "ast_i2c", (void *)AST_I2C_BASE ) < 0 )
#else
    if( request_irq(dt_irq, i2c_handler, SA_SHIRQ, "ast_i2c", (void *)AST_I2C_BASE ) < 0 )
#endif
     {
...
i2c_as_exit( void )
{
	...
	if (CONFIG_SPX_FEATURE_I2C_BUS_DISABLE_MASK & (1 << i)) continue;
        ...
        /* Disable the irq */
        disable_irq( dt_irq );
        free_irq(dt_irq, (void *)AST_I2C_BASE );
#if defined(GROUP_AST1070_COMPANION)
