...
static void  gpio_hw_exit (void)
{
	...
	{
		free_irq(gpio_irq,dev->ast2050_gpio_addr);
		#if defined(GROUP_AST2300) || defined(GROUP_AST2400) || defined(GROUP_AST2500)
		free_irq(sgpio_master_irq,dev->ast2050_gpio_addr);
		#endif
		GPIO_IRQ_ENABLED = 0;
...
int ast2050_regsensorints ( void* gpin_data, uint32_t total_interrupt_sensors, void* sensor_itr)
{
	...
	if(!GPIO_IRQ_ENABLED) {
		...
		if( request_irq(gpio_irq, gpio_intr_handler, IRQF_SHARED, AST_GPIO_DEV_NAME, gpio->ast2050_gpio_addr) < 0 ) {
	...
	#if defined(GROUP_AST2300) || defined(GROUP_AST2400) || defined(GROUP_AST2500)
		//if(bank >= BANK_EXTSGPIO) {
            sgpio_master_irq = GetIrqFromDT("ami_sgpio_master", IRQ_SGPIO_MASTER);
			if( request_irq(sgpio_master_irq, gpio_intr_handler, 0, AST_SGPIO_DEV_NAME, gpio->ast2050_gpio_addr) < 0 ) {
				printk( KERN_ERR "Request for irq %d failed\n", IRQ_SGPIO_MASTER );
			      if(hw_intr_sensors != NULL)
			          kfree(hw_intr_sensors);
			      hw_intr_sensors = NULL;
