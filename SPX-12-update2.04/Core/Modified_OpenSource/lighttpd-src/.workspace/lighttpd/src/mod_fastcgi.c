...
typedef struct {
	 ...
	 * - more secure (you have filecperms to play with)
...
static int fcgi_spawn_connection(server *srv,
	...
	if (-1 == connect(fcgi_fd, fcgi_addr, servlen)) {
		...
		/* reopen socket */
		if (-1 == (fcgi_fd = socket(socket_type, SOCK_STREAM, 0))) {	/* Unreleased Resource: False Positive */
			log_error_write(srv, __FILE__, __LINE__, "ss",
...
static int update_mapping(server *srv) {
	...
	if(mfile) {
		...
		while (!feof(mfile)) {
			...
			if(fscanf(mfile, "%s %lu", url, &size) == 2) {
...
static void handle_max_upload_size(server *srv, connection *con) {
	if(con->http_status != 413 && con->request.content_length > 0) {
		regex_t re;
		regmatch_t match[1];
		unsigned long size = 0;
		map *temp;
		...
		if(update_mapping(srv) != -1) {
		    ...
		    for(temp = last_map; temp != NULL; temp = temp->prev) {
		    	...
		    	if(regexec(&re, con->request.uri->ptr, 1, match, 0) == 0) {
					size = temp->size;
					break;
				}
			}
