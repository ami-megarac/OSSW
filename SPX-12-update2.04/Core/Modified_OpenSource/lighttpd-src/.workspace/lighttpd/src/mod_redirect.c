...
# define N 10
		...
		if (port != 443){
			memset (kv->value->ptr, 0, strlen (kv->value->ptr));
			sprintf (kv->value->ptr,"https://%1:%d$1", port);
			pattern_len = strlen(kv->value->ptr) + 1;
		}
		if(srv->srvconf.port == 80 && port != 443){
			memset (kv->value->ptr, 0, strlen (kv->value->ptr));
			sprintf (kv->value->ptr,"https://%0:%d$0", port);
			pattern_len = strlen(kv->value->ptr) + 1;
		}
		if(srv->srvconf.port == 80 && port == 443){
                        memset (kv->value->ptr, 0, strlen (kv->value->ptr));
                        sprintf (kv->value->ptr,"https://%0:443$0");
                        pattern_len = strlen(kv->value->ptr) + 1;
