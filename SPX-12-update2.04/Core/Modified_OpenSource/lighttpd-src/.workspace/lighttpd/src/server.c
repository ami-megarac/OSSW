...
#undef CLEAN
	...
	if ((NULL != (frandom = fopen("/dev/random", "rb")) || NULL != (frandom = fopen("/dev/urandom", "rb")))
	            && 1 == fread(srv->entropy, sizeof(srv->entropy), 1, frandom)) {
		unsigned int e;
		memcpy(&e, srv->entropy, sizeof(e));
		srand(e);
  ...
  if(g_serviceconf.SecureAccessPort != NA_SERVICE_PORT_BYTE)
  {
      for (i = 1; i < srv->config_context->used; i++)
      {
          ...
          if ((j == srv->srv_sockets.used) && (s->ssl_enabled == 1))
          {
              ...
              char *sap = NULL;
              bport = buffer_init();
              buffer_copy_string_buffer(bport,dc->string);
			...
			if (min_ts != srv->cur_ts) {
				...
				for (ndx = 0; ndx < conns->used; ndx++) {
					...
					if (con->state == CON_STATE_READ ||
						...
						// Manish: SSL_read() not done completely in prev read ssl. 
						// Add it to joblist incase fdevent doesnot occur on this con 
