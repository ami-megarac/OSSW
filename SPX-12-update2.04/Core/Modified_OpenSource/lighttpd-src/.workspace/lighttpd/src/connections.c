...
int connection_close(server *srv, connection *con) {
	...
	if (srv_sock->is_ssl) {
		...
		if(con->dupped != 1){
			if (con->bio) BIO_free_all(con->bio);
		}
		con->bio = NULL;
	}
#endif
...
#else
if (!((con->dupped ==1 ) &&(1 == srv_sock->is_ssl)) ){
	if (close(con->fd)) {
		log_error_write(srv, __FILE__, __LINE__, "sds",
				"(warning) close:", con->fd, strerror(errno));
	}
			...
			/*Blocking the IP where the thc-ssl-dos flood is running*/
				...
				{							
					if (ipversion == AF_INET) {
						sprintf (cmd, "iptables -A INPUT -p tcp -s %s --dport %d -j DROP", ipaddr,port);
					}
#ifdef HAVE_IPV6
					else if (ipversion == AF_INET6) {
						sprintf (cmd, "ip6tables -A INPUT -p tcp -s %s --dport %d -j DROP", ipaddr, port);
					}
#endif							
...
found_header_end:
	...
	case CON_STATE_READ_POST:{
		...
		if (HANDLER_ERROR == (r = plugins_call_handle_readpost(srv, con))) {
	
			printf("\n<<<<<<<<  HANDLER_ERROR>>>>\n");
			if (con->http_status) {
                		// plugin has set some http_status then. So, move to HANDLE_REQUEST to generate response
				printf("\n<<<<<<<<<<<<http_status set. HANDLE_REQUEST--->%d>>>>>>>\n",con->http_status);
		                connection_set_state(srv, con, CON_STATE_HANDLE_REQUEST);
            		}
            		else {
...
static handler_t connection_handle_fdevent(server *srv, void *context, int revents) {
	...
	if (con->state == CON_STATE_READ ||
	    ...
	    con->state == CON_STATE_READ_CONTINUOUS_WEB_APP_PROXY) {
			...
			case HANDLER_FINISHED:
				if (con->mode == DIRECT) {
					if (con->http_status == 404 ||
					    con->http_status == 403||con->http_status == HTTP_FLASH_ERROR||
					    con->http_status == HTTP_SESSION_ERROR) {
						/* 404 error-handler */
