...
static int ast_kcs_module_init(void)
{
	...
	kcs_irq = GetIrqFromDT("ami_kcs", AST_KCS_IRQ);
    ret = request_irq(kcs_irq, ast_kcs_irq_handler, IRQF_SHARED, AST_KCS_DRIVER_NAME, (void *)AST_KCS_REG_VA_BASE);
...
#endif /* CONFIG_SPX_FEATURE_NUM_BMC_COMPANION_DEVICES >= 2 */
#else
    free_irq(kcs_irq, (void *)AST_KCS_REG_VA_BASE);
#endif
    unregister_hw_hal_module(EDEV_TYPE_KCS, ast_kcs_hal_id);
}
