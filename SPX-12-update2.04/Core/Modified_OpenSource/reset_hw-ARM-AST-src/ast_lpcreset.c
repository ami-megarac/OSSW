...
static int __init ast_lpcreset_module_init(void)
{
	...
	reset_irq = GetIrqFromDT("ami_lpc", AST_LPCRESET_IRQ);
    ret = request_irq(reset_irq, ast_lpcreset_irq_handler, IRQF_SHARED, AST_LPCRESET_DRIVER_NAME, (void *)(AST_LPCRESET_REG_VA_BASE+1));
    if (ret < 0) {
...
#else
    free_irq(reset_irq, (void *)(AST_LPCRESET_REG_VA_BASE+1));
#endif
out_irq_init:
