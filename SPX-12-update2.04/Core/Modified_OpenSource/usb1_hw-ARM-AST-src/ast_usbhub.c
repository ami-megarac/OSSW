...
int ast_usbhub_module_init(void)
{
	...
	hub_irq = GetIrqFromDT("ami_usbhub", AST_USBHUB_IRQ);
#if (LINUX_VERSION_CODE < KERNEL_VERSION(5,2,0))
	ret = request_irq(hub_irq, ast_usbhub_irq_handler, IRQF_DISABLED, ast_usbhub_driver_name, NULL);
#else
	ret = request_irq(hub_irq, ast_usbhub_irq_handler,0, ast_usbhub_driver_name, NULL);
#endif
	if (ret != 0) {
