...
static struct ctl_table CdromctlTable [] =
{
        ...
        {.procname="DebugLevel",.data=&(TDBG_FORM_VAR_NAME(CDROM)),.maxlen=sizeof(int),.mode=0644,.proc_handler=&proc_dointvec},
#else 
...
CdromIoctl (unsigned int cmd,unsigned long arg, int* RetVal)
{
	...
	case USB_CDROM_EXIT:
		...
		BotIfData = (BOT_IF_DATA*) UsbCore.CoreUsbGetInterfaceData (DevNo, IfNum);
		EventStatus = &(BotIfData->EventStatus[Instance]);
		EventStatus->EventCode   = 0x03; 
		EventStatus->MediaStatus = 0x0; 
#endif
...
init_cdrom_device_module (void)
{
	...
	{
		if(DevInfo[devnum].UsbDevice.UsbHwSetCurrentDevType (devnum, cdrom_device_driver.descriptor))
		{
