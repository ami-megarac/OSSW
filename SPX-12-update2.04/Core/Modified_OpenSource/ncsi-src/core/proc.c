...
static struct ctl_table SysNcsiTable[] = 
{
    ...
    {.procname="UserSetLink"  ,  	.data=&UserSetLink,   		.maxlen=sizeof(int), .mode=0644, .proc_handler=&proc_setlink },
    ...
    {.procname="UserCommand"   ,  	.data=UserCommand,    		.maxlen=NCSI_COMMAND_SIZE, .mode=0644, .proc_handler=&proc_usercommand },
    {.procname="UserLastResponse",  .data=UserLastResponse,    	.maxlen=NCSI_COMMAND_SIZE, .mode=0644, .proc_handler=&proc_userresponse },
...
#endif
{
	...
	*lenp = 0;
	...
	{
		read_flag = 0;
		memset (UserLastResponse, 0, sizeof(UserLastResponse));
		return retval;
	}
