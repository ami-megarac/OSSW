...
CheckIfAEN(NCSI_IF_INFO *info, struct sk_buff *skb)
{
	...
	if (is_async_reset == 1){
		printk ("NCSI : Async reset in progress, ignore AEN...\n");
		return 1;
	}
#endif	
	switch (aen->AEN_Type)
...
NCSI_Issue_GetParameters(NCSI_IF_INFO *info, UINT8 PackageID, UINT8 ChannelID, UINT32 *ConfigFlags, UINT32 *AENControl)
{
	...
	GET_PARAMETERS_RES_PKT *ResPkt = NULL;
	GET_MARVELL_PARAMETERS_RES_PKT *Marvell_ResPkt = NULL;
	GET_BROADCOM_PARAMETERS_RES_PKT *Broadcom_ResPkt = NULL;
	...
	if (info->IANA_ManID == 0x210f0000) // Marvell
	{
		...
		{
			printk("NCSI(%s): Expected Response Size = %zu Got %d\n",info->dev->name,
				sizeof(GET_MARVELL_PARAMETERS_RES_PKT),info->RecvLen);
			retval = NCSI_ERR_RESPONSE;
			goto exit;
		}
	}
	else if (info->IANA_ManID == 0x3d110000) // Broadcom
	{
		...
		{
			printk("NCSI(%s): Expected Response Size = %zu Got %d\n",info->dev->name,
				sizeof(GET_BROADCOM_PARAMETERS_RES_PKT),info->RecvLen);
			retval = NCSI_ERR_RESPONSE;
			goto exit;
		}
	}
	...
	if (info->IANA_ManID == 0x210f0000) // Marvell
	{
		...
		if (retval == NCSI_ERR_SUCCESS)
		{
			...
			*ConfigFlags = be32_to_cpu(Marvell_ResPkt->ConfigurationFlags);
			*AENControl = be32_to_cpu(Marvell_ResPkt->AENControl);
	...
	else if (info->IANA_ManID == 0x3d110000) // Broadcom
	{
		...
		if (retval == NCSI_ERR_SUCCESS)
		{
			...
			*ConfigFlags = be32_to_cpu(Broadcom_ResPkt->ConfigurationFlags);
			*AENControl = be32_to_cpu(Broadcom_ResPkt->AENControl);
				/*TODO: Send back other useful info ? */
		}
	}
	else
