...
static unsigned long LastKeybdEventTimeStamp [MAX_HID_IFACES] = {0,0,0,0}; 
...
SendKeybdData (uint8 DevNo,uint8 IfNum, uint8 *Buff)
{
	...
	if (LastKeybdEventTimeStamp[IfNum]) 
	{ 
		CurrentTimeStamp[IfNum] = jiffies; 
		if (CurrentTimeStamp[IfNum] > LastKeybdEventTimeStamp [IfNum]) 
		{ 
			if (((CurrentTimeStamp [IfNum] - LastKeybdEventTimeStamp [IfNum]) * 1000 / HZ) > 1000) 
			{ 
				printk ("Keybd event timed out..\n"); 
				KeybdQueue[IfNum].WaitForHostToReceiveKeybdData = 0; 
			} 
	...
	if (0 == KeybdQueue[IfNum].Size % DATA_QUEUE_SIZE)
	{
	...
	if (KeybdQueue[IfNum].WaitForHostToReceiveKeybdData == 0)
	{
		...
		{ 
			LastKeybdEventTimeStamp [IfNum] = jiffies; 
		} 
		spin_unlock_irqrestore (&KeybdQueue[IfNum].Lock, Flags);
	}
	else
...
KeybdTxHandler (uint8 DevNo, uint8 IfNum, uint8 EpNum)
{
	...
	LastKeybdEventTimeStamp [IfNum] = 0;
	if (KeybdQueue[IfNum].WaitForHostToReceiveKeybdData == 0)
	{
