...
uint32_t device_data_check(unsigned int* read_buffer, unsigned int* compare_data, unsigned int bits_leng ){
	...
	// Calculate - the sum of all data (unit is byte)
		if((JTAG_device_information.Device_Column_Length%32) != 0){			
			leng = (JTAG_device_information.Device_Column_Length/32+1)*JTAG_device_information.Device_Row_Length;			
			for (i = 0; i < leng; i++) {
				if((i /8 % 2) == 0){
					chksum += (unsigned char)((read_buffer[i] >> 24) & 0xFF);
					chksum += (unsigned char)((read_buffer[i] >> 16) & 0xFF);
					chksum += (unsigned char)((read_buffer[i] >>  8) & 0xFF);
					chksum += (unsigned char)((read_buffer[i]	   ) & 0xFF);
				}
				else {				
					chksum += (((read_buffer[i] & 0x0F)<<4)|((read_buffer[i] & 0xF0)>>4));
					chksum += ((((read_buffer[i] >>  8) & 0x0F) <<4)|(((read_buffer[i]>>8) & 0xF0)>>4));
					chksum += ((((read_buffer[i] >>  16) & 0x0F)<<4)|(((read_buffer[i]>>16) & 0xF0)>>4));					
					chksum += ((((read_buffer[i] >>  24) & 0x0F)<<4)|(((read_buffer[i]>>24) & 0xF0)>>4));
				}
				  
				  //printk("Calculate Done1.[%08x] at %d\n",chksum, i);
			}
		} 
		else {
			 leng = (bits_leng  + 31) / 32;
		for (i = 0; i < leng; i++) {
...
//  uint32_t Tdo_32bits;
  ...
  //---------------------------------------------------------
	switch(cmd){
      ...
      *buf = device_data_check(get_jtag_read_buffer(), 0, JTAG_device_information.Device_All_Bits_Length);
      ...
      if ( DEVICE_FAMILY_FUNC(DeviceFamily_Erase) != 0 ) {
        ...
        ret = 1; /* Fortify issue :: False Positive */ 
	...
	unsigned char reduant_flag=0;
        // initial all variables
        row_count = JTAG_device_information.Device_Row_Length;
        col_count = JTAG_device_information.Device_Column_Length;
	new_buf = kmalloc (col_count/8 + 1, GFP_DMA|GFP_KERNEL);

        
        while (row_count > 0) {
		  //initial new_buf
		  memset(new_buf, 0, col_count/8+1);
		  
        	//! Shift in DATA SHIFT(0x02) instruction
          SIR(8, ISC_DATA_SHIFT);
		  if(col_count%32 != 0)
          {
		    bufPtr = ((unsigned char*)buf)+buf_index;			
			memcpy((void*)new_buf, (void*)bufPtr, col_count/8+1);

		    if(!reduant_flag)
		    {		    
			  ((unsigned char*)new_buf)[col_count/8] = (((unsigned char*)new_buf)[col_count/8]) & 0x0f;
			  
			  reduant_flag = 1;
			  
			  buf_index += col_count/8/*-1*/;
		    }
		    else
		    {
              for(loop_i = 0; loop_i < col_count/8; loop_i++)
			  	((unsigned char*)new_buf)[loop_i] = ((((unsigned char*)new_buf)[loop_i+1] & 0x0f) << 4) |((((unsigned char*)new_buf)[loop_i] & 0xf0) >> 4);

			  ((unsigned char*)new_buf)[col_count/8] = (((((unsigned char*)new_buf)[col_count/8]) & 0xf0) >>4);			  
...
uint32_t hw_device_ctl(unsigned int cmd ,unsigned int *buf,unsigned long data_size){
  ...
  if(pDevice == NULL)    /* NULL Dereference */
  {
	  printk("Error : Dereferencing Null Pointer File:%s Line: %d  Function : %s\n",__FILE__, __LINE__, __func__);
      return 1;
  }
...
uint32_t load_device_info(unsigned long id_code){
  ...
  const JTAG_DEVICE_INFO MachXO_information[]=
  {
    ...
    { //LATTICE_CPLD_LCMXO2_2000HC_DEVNAME: 
      .Device_Name    =               LATTICE_CPLD_LCMXO2_2000HC_DEVNAME,
      .Device_ID      =               LATTICE_CPLD_LCMXO2_2000HC_IDCODE,
      .Device_Column_Length   =       LATTICE_COLS_LCMXO2_2000HC_LENGTH,
      .Device_Row_Length      =       LATTICE_ROWS_LCMXO2_2000HC_LENGTH,
      .Device_All_Bits_Length =       LATTICE_BITS_LCMXO2_2000HC_LENGTH,
