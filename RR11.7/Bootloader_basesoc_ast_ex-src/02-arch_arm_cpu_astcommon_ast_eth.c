--- u-boot-2013.07/arch/arm/cpu/astcommon/ast_eth.c	2017-01-31 11:06:59.285364097 -0500
+++ uboot.new/arch/arm/cpu/astcommon/ast_eth.c	2017-01-31 11:06:51.181323738 -0500
@@ -0,0 +1,904 @@
+#include <common.h>
+
...
+
+#define EXPANSION_REG       0x17 /*Expansion Register Access Register*/
+
+/* bits of MACCR */
...
+
+#define VIRTUAL_ETH_MAC_OFFSET_FLAG	0x80
...
+	if (id == 0) {
+		eeprom_read(CONFIG_SYS_EEPROM_ADDR, CONFIG_SPX_FEATURE_I2C_EEPROM_MACADDR_OFFSET, mac_address, ETH_ALEN);
+	} 
+#endif
+#ifdef CONFIG_SPX_FEATURE_I2C_EEPROM_MAC1ADDR_OFFSET
+	else if (id == 1) {
+		eeprom_read(CONFIG_SYS_EEPROM_ADDR, CONFIG_SPX_FEATURE_I2C_EEPROM_MAC1ADDR_OFFSET, mac_address, ETH_ALEN);
+	}
+#endif
...
+    	else if (id == 2) {
+        	eeprom_read(CONFIG_SYS_EEPROM_ADDR, CONFIG_SPX_FEATURE_I2C_EEPROM_MAC2ADDR_OFFSET, mac_address, ETH_ALEN);    
+    	}
+	#endif
+	#ifdef CONFIG_SPX_FEATURE_I2C_EEPROM_MAC3ADDR_OFFSET
+    	else if (id == 3) {
+        	eeprom_read(CONFIG_SYS_EEPROM_ADDR, CONFIG_SPX_FEATURE_I2C_EEPROM_MAC3ADDR_OFFSET, mac_address, ETH_ALEN);    
+    	}
+	#endif
+#endif //NIC COUNT >= 3
+
+#if defined(CONFIG_VIRTUAL_ETH)
+	if ((id & VIRTUAL_ETH_MAC_OFFSET_FLAG) == VIRTUAL_ETH_MAC_OFFSET_FLAG)
+	{
+		virteth_id = (id & 0x7F);
+	#ifdef CONFIG_SPX_FEATURE_I2C_EEPROM_VMACADDR_OFFSET
+		if (virteth_id == 0) {
+			eeprom_read(CONFIG_SYS_EEPROM_ADDR, CONFIG_SPX_FEATURE_I2C_EEPROM_VMACADDR_OFFSET, mac_address, ETH_ALEN);
+		} 
+	#endif
+	#ifdef CONFIG_SPX_FEATURE_I2C_EEPROM_VMAC1ADDR_OFFSET
+		else if (virteth_id == 1) {
+			eeprom_read(CONFIG_SYS_EEPROM_ADDR, CONFIG_SPX_FEATURE_I2C_EEPROM_VMAC1ADDR_OFFSET, mac_address, ETH_ALEN);
+		}
+	#endif
+	#ifdef CONFIG_SPX_FEATURE_I2C_EEPROM_VMAC2ADDR_OFFSET
+    		else if (virteth_id == 2) {
+        		eeprom_read(CONFIG_SYS_EEPROM_ADDR, CONFIG_SPX_FEATURE_I2C_EEPROM_VMAC2ADDR_OFFSET, mac_address, ETH_ALEN);    
+    		}
+	#endif
+	#ifdef CONFIG_SPX_FEATURE_I2C_EEPROM_VMAC3ADDR_OFFSET
+	 	else if (virteth_id == 3) {
+	        	eeprom_read(CONFIG_SYS_EEPROM_ADDR, CONFIG_SPX_FEATURE_I2C_EEPROM_VMAC3ADDR_OFFSET, mac_address, ETH_ALEN);    
+    		}
+	#endif
+	}
+#endif //CONFIG_VIRTUAL_ETH
+
...
+#if defined(CONFIG_VIRTUAL_ETH)
+		if (virteth_id == 0) {
+			memcpy(bis->bi_vnetaddr, mac_address,6);
+			setenv("vethaddr", ethaddr_env);
+		}
+		else if (virteth_id == 1) {
+			memcpy(bis->bi_vnet1addr, mac_address,6);
+			setenv("veth1addr", ethaddr_env);
+		} 
+		else if (virteth_id == 2) {
+			memcpy(bis->bi_vnet2addr, mac_address,6);
+			setenv("veth2addr", ethaddr_env);
+		}
+		else if (virteth_id == 3) {
+			memcpy(bis->bi_vnet3addr, mac_address,6);
+			setenv("veth3addr", ethaddr_env);
+		}
+#endif // CONFIG_VIRTUAL_ETH
+
+		saveenv();
...
+#if defined(CONFIG_AST2500EVB) || defined(CONFIG_AST2530EVB) || defined(CONFIG_AST3200EVB)
+	/* For some SOC, below reg setting is handled by ASPEED in platform.S. Please contact with ASPEED before try to set this reg in here */
+#else
...
+       { 
+               scu_reg = *((volatile unsigned long *) SCU_MISC_MULTI_PIN5_REG); 
+               *((volatile unsigned long *) SCU_MISC_MULTI_PIN5_REG) = (scu_reg | 0x00000004); 
+       }  
+#endif
+
+//#if defined(CONFIG_AST2500EVB) || defined(CONFIG_AST2530EVB)
...
+	}
+
+#if defined(CONFIG_MACADDR_IN_EEPROM) && defined(CONFIG_VIRTUAL_ETH)
+	#ifdef CONFIG_SPX_FEATURE_I2C_EEPROM_VMACADDR_OFFSET
+	ast_eth_read_addr_from_eeprom(0|VIRTUAL_ETH_MAC_OFFSET_FLAG, bis);
+	#endif
+	#ifdef CONFIG_SPX_FEATURE_I2C_EEPROM_VMAC1ADDR_OFFSET
+	ast_eth_read_addr_from_eeprom(1|VIRTUAL_ETH_MAC_OFFSET_FLAG, bis);
+	#endif
+	#ifdef CONFIG_SPX_FEATURE_I2C_EEPROM_VMAC2ADDR_OFFSET
+	ast_eth_read_addr_from_eeprom(2|VIRTUAL_ETH_MAC_OFFSET_FLAG, bis);
+	#endif
+	#ifdef CONFIG_SPX_FEATURE_I2C_EEPROM_VMAC3ADDR_OFFSET
+	ast_eth_read_addr_from_eeprom(3|VIRTUAL_ETH_MAC_OFFSET_FLAG, bis);
+	#endif
+#endif
+
+	return 0;
